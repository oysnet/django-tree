...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:tree.1c6c558e014f.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var i='/',h='tree/js/app/build/resource',g='selection[0]',f="horizontal",e='dropend',d="tree.Application",c="tree-container",b='node',a='undefined';
qx.Class.define(d,{extend:qx.application.Inline,members:{main:function(){if(typeof (STATIC_URL)!==a){var l=STATIC_URL;

if(l.substr(-1)!==i){l+=i;
}qx.$$libraries.qx.resourceUri=l+h;
qx.$$libraries.tree.resourceUri=l+h;
}qx.application.Inline.prototype.main.call(this);
var j=document.getElementById(c);
var k=new qx.ui.root.Inline(j,true,true);
k.setLayout(new qx.ui.layout.Grow);
var m=new qx.ui.splitpane.Pane(f);
k.add(m);
var t=new tree.ui.tree.Tree();
var n=new tree.ui.gallery.Gallery();
m.add(t);
m.add(n,5);
t.bind(g,n,b);
t.addListener(e,n.refreshNodeItems,n);
}}});
})();
...
(function(){var m='success',k='error',j="qx/tree-nodes",h='execute',g='continue',f="html/cells",d='Rename',c='body',b='unselectable',a='custom/8/delete.png',L='Give a name to the folder',K='Delete',J="default",I='custom/8/add.png',H='qx.event.type.Data',G="drop",F="open",E='Add',D='Give a name to the new folder',C="tree.ui.tree.Tree",u='dragover',v='dropend',s='custom/8/rename.png',t="move",q="children",r='undefined',o="name",p="dragstart",w="",x='Delete the node named "%1"',z='beforeContextmenuOpen',y='Add a folder',B="Root",A="dragend";
qx.Class.define(C,{extend:qx.ui.tree.VirtualTree,events:{dropend:H},construct:function(){var M={label:B,children:[],icon:J,loaded:true,pk:null};
M=qx.data.marshal.Json.createModel(M,true);
this.__nr=M;
this.__ns={};
this.__ny(M,true);
qx.ui.tree.VirtualTree.call(this,M,o,q);
this.setWidth(400);
this.setHideRoot(true);
this.setShowTopLevelOpenCloseIcons(true);
var N={configureItem:qx.lang.Function.bind(function(O){O.setDraggable(true);
O.setDroppable(true);
O.addListener(p,function(e){e.addAction(t);
e.addType(j);
qx.bom.Collection.query(c).addClass(b);
});
O.addListener(A,function(e){qx.bom.Collection.query(c).removeClass(b);
});
O.addListener(u,function(event){var R=event.getRelatedTarget();

if(R===null){event.preventDefault();
return;
}var P=event.getTarget().getModel();

if(event.supportsType(f)){if(P==this.getSelection().getItem(0)){event.preventDefault();
}}else{var Q=R.getModel();

if(P.getPk()===Q.getPk()||this.getAncestors(P).indexOf(Q)!==-1){event.preventDefault();
}}
if(this.isNodeOpen(P)===false){this.openNode(P);
}},this);
O.addListener(G,function(e){var S=[f,j];

for(var i=0,l=S.length;i<l;i++){if(e.supportsType(S[i])){var T=e.getTarget().getModel();

switch(S[i]){case f:var Y=e.getData(S[i]);
var X={node:T.getPk(),items:[]};

for(var i=0,l=Y.length;i<l;i++){X.items.push(Y[i].itemPk);
}var W=new tree.ui.popup.Waiting();
var U=new tree.io.rest.Item();
U.addListener(m,function(){W.close();
this.fireDataEvent(v,T);
},this);
U.addListener(k,function(){W.close();
new tree.ui.popup.Error(this.tr('something went wrong'));
},this);
U.moveToNode(X);
break;
case j:var V=e.getRelatedTarget().getModel();
var X={node:V.getPk(),target:T.getPk()};
var W=new tree.ui.popup.Waiting();
var U=new tree.io.rest.Node();
U.addListener(m,function(event){var ba=event.getData().position;
V.setName(event.getData().name);
W.close();
this.getParent(V).getChildren().remove(V);
V.setParent(T.getPk());
T.getChildren().insertAt(ba,V);
this.refresh();
this.openNodeAndParents(V);
this.setSelection(new qx.data.Array([V]));
},this);
U.addListener(k,function(){W.close();
new tree.ui.popup.Error(this.tr('something went wrong'));
},this);
...
},this),bindItem:qx.lang.Function.bind(function(bb,bc,bd){bb.bindDefaultProperties(bc,bd);
bb.bindProperty(w,F,{converter:qx.lang.Function.bind(function(parent,be,bf,bg){var bh=bg.isOpen();

if(bh&&!parent.getLoaded()){this.__ny(parent);
}return bh;
},this)},bc,bd);
},this)};
this.setDelegate(N);
this.__nu();
},members:{__ns:null,__nr:null,__nt:null,__nu:function(){var bl=new qx.ui.menu.Menu();
var bj=new qx.ui.menu.Button(this.tr(E),I);
bl.add(bj);
bj.addListener(h,this.__nx,this);
var bk=new qx.ui.menu.Button(this.tr(d),s);
bl.add(bk);
bk.addListener(h,this.__nw,this);
var bi=new qx.ui.menu.Button(this.tr(K),a);
this.__nt=bi;
bl.add(bi);
bi.addListener(h,this.__nv,this);
this.addListener(z,function(){if(this.getSelection().getItem(0).getChildren().getLength()>0){this.__nt.setEnabled(false);
}else{this.__nt.setEnabled(true);
}},this);
this.setContextMenu(bl);
},__nv:function(){var bm=this.getSelection().getItem(0);
var bn=new tree.ui.popup.Confirm();
bn.setText(this.tr(x,bm.getName()));
bn.addListener(g,function(){var bo=new tree.io.rest.Node();
var bp=new tree.ui.popup.Waiting();
bo.addListener(k,function(event){bp.close();
new tree.ui.popup.Error(event.getRequest().getResponseText());
},this);
bo.addListener(m,function(parent,event){bp.close();
var parent=this.getParent(bm);
var bq=parent.getChildren();
bq.remove(bm);
delete this.__ns[bm.getPk()];
this.openNode(parent);
this.setSelection(new qx.data.Array([parent]));
},this);
bo.del({id:bm.getPk()});
},this);
},__nw:function(){var br=this.getSelection().getItem(0);
var bs=new tree.ui.popup.EditCreate();
bs.setCaption(this.tr(d));
bs.setText(this.tr(L));
bs.setFieldNameValue(br.getName());
bs.addListener(g,function(event){var bu=event.getData();
var bt=new tree.io.rest.Node();
var bv=new tree.ui.popup.Waiting();
bt.addListener(k,function(event){bv.close();
new tree.ui.popup.Error(event.getRequest().getResponseText());
},this);
bt.addListener(m,function(parent,event){bv.close();
br.setName(bu);
},this);
bt.update({id:br.getPk(),name:bu});
});
},__nx:function(){var bw=new tree.ui.popup.EditCreate();
bw.setCaption(this.tr(y));
bw.setText(this.tr(D));
bw.addListener(g,function(event){var by=event.getData();
var parent=this.getSelection().getItem(0);
var bx=new tree.io.rest.Node();
var bz=new tree.ui.popup.Waiting();
bx.addListener(k,function(event){bz.close();
new tree.ui.popup.Error(event.getRequest().getResponseText());
},this);
bx.addListener(m,qx.lang.Function.bind(function(parent,event){bz.close();
var bA=qx.data.marshal.Json.createModel(event.getData(),true);

if(this.isNodeOpen(parent)===true){this.__ns[bA.getPk()]=bA;
parent.getChildren().insertAt(bA.getPosition(),bA);
}else{this.openNode(parent);

if(parent.getChildren().getLength()==0){parent.setLoaded(true);
this.__ny(parent);
}}},this,parent));
bx.create({parent:parent.getPk(),name:by});
},this);
},__ny:function(parent,bB){bB=typeof (bB)!==r?bB:false;
var bC=new tree.io.rest.Node();
bC.addListener(m,qx.lang.Function.bind(function(parent,event){var bD=qx.data.marshal.Json.createModel(event.getData(),true);

for(var i=0,l=bD.getLength();i<l;i++){var n=bD.getItem(i);
this.__ns[n.getPk()]=n;
}parent.setChildren(bD);

if(this.isNodeOpen(parent)===false){this.openNode(parent);
}
if(bB===true&&bD.getLength()>0){this.setSelection(new qx.data.Array([bD.getItem(0)]));
}parent.setLoaded(true);
},this,parent));

if(parent.getPk()===null){bC.getRoot();
}else{bC.getChildren({id:parent.getPk()});
}},getAncestors:function(bE){var bF=[];
var n=bE;

while((n=this.getParent(n))!==null){bF.push(n);
}return bF;
},getParent:function(bG){if(this.__nr==bG){return null;
}else if(bG.getParent()!==null){return this.__ns[bG.getParent()];
...
}},members:{__nz:null,__nA:function(n){return qx.Bootstrap.getKeys(n).sort().join(e);
...
}if(this.__nz&&this.__nz.getModelClass&&this.__nz.getModelClass(r)!=null){return;
}var y={};
var x={__nB:this.__nB};
...
if(p){y[s].apply=a;
}if(this.__nz&&this.__nz.getValidationRule){var u=this.__nz.getValidationRule(r,s);

if(u){y[s].validate=f+s;
x[f+s]=u;
}}}if(this.__nz&&this.__nz.getModelSuperClass){var w=this.__nz.getModelSuperClass(r)||qx.core.Object;
}else{var w=qx.core.Object;
}var t=[];

if(this.__nz&&this.__nz.getModelMixins){var v=this.__nz.getModelMixins(r);
if(!qx.lang.Type.isArray(v)){if(v!=null){t=[v];
}}}if(p){t.push(qx.data.marshal.MEventBubbling);
}var q={extend:w,include:t,properties:y,members:x,destruct:this.__nC};
qx.Class.define(j+r,q);
},__nC:function(){var z=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var A in z){this.__nB(this.get(z[A].name));
}},__nB:function(B){if(!(B instanceof qx.core.Object)){return ;
}if(B.isDisposed()){return;
}B.dispose();
},__nD:function(C){var D;
if(this.__nz&&this.__nz.getModelClass){D=this.__nz.getModelClass(C);
}
...
}},destruct:function(){this.__nz=null;
}});
})();
...
qx.Class.define(f,{statics:{__nE:/\s+/g,__nF:/^\s+|\s+$/g,add:qx.lang.Object.select(qx.core.Environment.get(s)?q:r,{"native":function(t,name){t.classList.add(name);
return name;
...
return M.className=M.className.replace(O,p).replace(this.__nF,p).replace(this.__nE,o);
...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__nG:false,__nH:null,__nI:null,__nJ:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===g)){return true;
}
if(C===j&&(D===m||D===g||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&(qx.core.Environment.get(d)<9||(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)<9))){if(!E.__nK){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__nL:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(d)<9&&qx.core.Environment.get(A)<9){if(O.__nK){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__nM:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__nI){this.__nI=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__nJ){this.__nJ=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__nG=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__nG=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__nH&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__nH);
...
if(!this.__nG||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__nH=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
if(bf&&bf.nodeType){return this.__nR([bf]);
}else{return bf;
}}return null;
};
};
...
},__nN:/^[^<]*(<(.|\s)+>)[^>]*$|^#([\w-]+)$/,create:function(bp,bq){var bs=qx.bom.Collection;
if(bp.nodeType){return new bs(bp);
}else if(typeof bp===I){var br=bs.__nN.exec(bp);
...
},__nP:/^.[^:#\[\.,]*$/,not:function(bI){if(this.__nP.test(bI)){var bJ=qx.bom.Selector.matches(g+bI+d,this);
return this.__nR(bJ);
...
return this.__nR(qx.lang.Array.unique(bT));
...
if(this.length===1){return this.__nR(cb.query(bY,this[0]));
}else{var ca=[];

for(var i=0,l=this.length;i<l;i++){ca.push.apply(ca,cb.query(bY,this[i]));
}return this.__nR(qx.lang.Array.unique(ca));
...
},nextAll:function(cf){return this.__nQ(y,cf);
...
},prevAll:function(cj){return this.__nQ(c,cj);
...
},parents:function(cm){return this.__nQ(H,cm);
},siblings:function(cn){return this.__nQ(s,cn);
},__nQ:function(co,cp){var cr=[];
...
},__nR:function(ct){var cu=new qx.bom.Collection;
cu.__nO=this;
ct=Array.prototype.slice.call(ct,0);
cu.push.apply(cu,ct);
return cu;
},andSelf:function(){return this.add(this.__nO);
},end:function(){return this.__nO||new qx.bom.Collection();
},__nS:function(cv,cw){var cB=this[0];
...
},__nT:function(cG,cH){var cJ=qx.bom.Selector;
...
},append:function(cM){return this.__nS(arguments,this.__nU);
},prepend:function(cN){return this.__nS(arguments,this.__nV);
},__nU:function(cO,cP){cO.appendChild(cP);
},__nV:function(cQ,cR){cQ.insertBefore(cR,cQ.firstChild);
},appendTo:function(cS){return this.__nT(arguments,e);
},prependTo:function(cT){return this.__nT(arguments,J);
},before:function(cU){return this.__nS(arguments,this.__nW);
},after:function(cV){return this.__nS(arguments,this.__nX);
},__nW:function(cW,cX){cW.parentNode.insertBefore(cX,cW);
},__nX:function(cY,da){cY.parentNode.insertBefore(da,cY.nextSibling);
},insertBefore:function(db){return this.__nT(arguments,w);
},insertAfter:function(dc){return this.__nT(arguments,q);
...
}dd.map(this.__nY).append(this);
}return this;
},__nY:function(df){while(df.firstChild){df=df.firstChild;
}return df;
...
},replaceAll:function(di){return this.__nT(arguments,n);
...
qx.Class.define(n,{statics:{__oa:function(K,L,M){return M.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?K:L+z+M+H;
},__ob:{opt:[1,e,c],leg:[1,f,u],table:[1,h,y],tr:[2,r,E],td:[3,I,b],col:[2,G,C],def:qx.core.Environment.select(g,{"mshtml":[1,s,d],"default":null})},__oc:function(N,O){var U=O.createElement(J);
N=N.replace(/(<(\w+)[^>]*?)\/>/g,this.__oa);
...
if(typeof ba===m){ba=this.__oc(ba,X);
...
qx.Bootstrap.define(d,{construct:function(){this.__od=qx.Bootstrap.bind(this.__og,this);
this.__kH=document.createElement(f);
},statics:{TIMEOUT:15},members:{__oe:null,__of:null,__bD:null,__bE:null,__od:null,__kH:null,load:function(q,r,s){if(this.__oe){throw new Error("Another request is still running!");
}this.__oe=true;
this.__of=false;
...
},abort:function(){if(this.__oe){this.dispose(a);
}},dispose:function(status){if(this.__of){return;
}this.__of=true;
var x=this.__kH;
...
},__og:qx.event.GlobalError.observeMethod(function(e){var y=qx.bom.client.Engine.getName();
...
(function(){var b="qx.ui.window.Manager",a="__oh";
qx.Class.define(b,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__oh:null,setDesktop:function(c){this.__oh=c;
this.updateStack();
},getDesktop:function(){return this.__oh;
},changeActiveWindow:function(d,e){if(d){this.bringToFront(d);
d.setActive(true);
}
if(e){e.resetActive();
}},_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this.__oh.forceUnblockContent();
var f=this.__oh.getWindows();
...
}}this.__oh.setActiveWindow(k);
},bringToFront:function(n){var o=this.__oh.getWindows();
var p=qx.lang.Array.remove(o,n);

if(p){o.push(n);
this.updateStack();
}},sendToBack:function(q){var r=this.__oh.getWindows();
...
qx.Mixin.define(b,{members:{__oi:function(m,n,o,p){var q=this.getChildrenContainer();

if(q===this){m=h+m;
}return (q[m])(n,o,p);
},getChildren:function(){return this.__oi(c);
},hasChildren:function(){return this.__oi(f);
},add:function(r,s){return this.__oi(j,r,s);
},remove:function(t){return this.__oi(a,t);
},removeAll:function(){return this.__oi(d);
},indexOf:function(u){return this.__oi(l,u);
},addAt:function(v,w,x){this.__oi(g,v,w,x);
},addBefore:function(y,z,A){this.__oi(i,y,z,A);
},addAfter:function(B,C,D){this.__oi(k,B,C,D);
},removeAt:function(E){return this.__oi(e,E);
}}});
})();
...
(function(){var l="move",k="Boolean",j="mouseup",i="mousedown",h="losecapture",g="qx.ui.core.MMovable",f="__ok",d="mousemove",c="maximized",b="__oj",a="move-frame";
qx.Mixin.define(g,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__oj:null,__ok:null,__ol:null,__om:null,__on:null,__oo:null,__op:null,__oq:false,__or:null,__os:0,_activateMoveHandle:function(m){if(this.__oj){throw new Error("The move handle could not be redefined!");
}this.__oj=m;
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__or=parent.getBlockerColor();
this.__os=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__oq=true;
}}this.__ol={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__oo=w.left;
this.__op=w.top;
this.__om=v.left-e.getDocumentLeft();
this.__on=v.top-e.getDocumentTop();
this.addState(l);
this.__oj.capture();
if(this.getUseMoveFrame()){this.__ou();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__ov(e);

if(this.getUseMoveFrame()){this.__ot().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__oq){parent.unblockContent();
parent.setBlockerColor(this.__or);
parent.setBlockerOpacity(this.__os);
this.__or=null;
this.__os=0;
this.__oq=false;
}}this.__oj.releaseCapture();
var z=this.__ov(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__ot().exclude();
}e.stopPropagation();
},__ow:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__ot().exclude();
...
},members:{__oR:null,_applyCloseOnEscape:function(h,i){if(h===true){this.__oR=this.addListener(c,this.__fR,this);
}else if(this.__oR!==null){this.removeListenerById(this.__oR);
this.__oR=null;
...
try{if(typeof r!==q){this.__pd(r);
}}catch(e){this.dispose();
throw e;
}},events:{"success":p,"actionSuccess":p,"error":p,"actionError":p},statics:{POLL_THROTTLE_LIMIT:100,POLL_THROTTLE_COUNT:30,REQUIRED:true},members:{__oS:null,__oT:null,__oW:null,__oU:null,__oV:null,__oX:null,configureRequest:function(s){this.__oX=s;
},_getRequest:function(){return new qx.io.request.Xhr();
},__oY:function(t){var u=this._getRequest();

if(!qx.lang.Type.isArray(this.__oS[t])){this.__oS[t]=[];
}this.__oS[t].push(u);
return u;
},map:function(v,w,x,y){this.__oT[v]=[w,x,y];
this.__oS[v]=[];
...
this[v].action=true;
},invoke:function(z,A){var D=this.__oY(z),G=this._getRequestConfig(z,A),A=A==null?{}:A,C=qx.lang.Object.clone(A);
this.__oT[z].params=A;
if(A){this.__pc(this.__oT[z][1]).forEach(function(H){delete C[H];
});
}this.__pa(A,G.check);

if(this.__oX){this.__oX.call(this,D,z,A);
}this.__pb(D,C,z,G);
...
},setBaseUrl:function(K){this.__oW=K;
},__pa:function(L,M){if(typeof M!==q){qx.lang.Object.getKeys(M).forEach(function(N){if(M[N]===qx.io.rest.Resource.REQUIRED&&typeof L[N]===q){throw new Error("Missing parameter '"+N+"'");
}if(!(M[N]&&typeof M[N].test==l)){return;
}if(!M[N].test(L[N])){throw new Error("Parameter '"+N+"' is invalid");
}});
}},__pb:function(O,P,Q,R){O.set({method:R.method,url:R.url});
...
if(this.__oS[U]){Y.forEach(function(ba){ba.abort();
});
}}},refresh:function(bb){this.invoke(bb,this.__oT[bb].params);
},poll:function(bc,bd,be){if(this.__oU[bc]){this.__oU[bc].dispose();
}if(typeof be==q){be=this.__oT[bc].params;
}this.invoke(bc,be);
var bf=this.__oU[bc]=new qx.event.Timer(bd);
bf.addListener(h,function bg(){var bh=this.__oS[bc][0];
...
}var bl=this.__oV[bi]=this.addListener(bi+o,function bp(){if(bk.isDisposed()){return;
}
if(!bj()){bn=new Date();
bk.refresh(bi);
}},bm);
this.invoke(bi);
return bl;
},_getRequestConfig:function(br,bs){var bu=this.__oT[br];

if(!qx.lang.Type.isArray(bu)){throw new Error("No route for action "+br);
}var bw=bu[0],bt=this.__oW!==null?this.__oW+bu[1]:bu[1],bx=bu[2],bv=this.__pc(bt);
bs=bs||{};
...
},__pc:function(bB){var bD=/\{(\w+)(=\w+)?\}/g,bE,bC=[];
while((bE=bD.exec(bB))){bC.push(bE[1]);
}return bC;
},__pd:function(bF){qx.lang.Object.getKeys(bF).forEach(function(bG){var bI=bF[bG],bJ=bI.method,bH=bI.url,bK=bI.check;
this.map(bG,bJ,bH,bK);
},this);
},__pe:function(bL){if(!this.constructor.$$events){this.constructor.$$events={};
}
if(!this.constructor.$$events[bL]){this.constructor.$$events[bL]=p;
}}},destruct:function(){var bM;

for(bM in this.__oS){if(this.__oS[bM]){this.__oS[bM].forEach(function(bP){bP.dispose();
});
}}
if(this.__oU){for(bM in this.__oU){var bO=this.__oU[bM];
bO.stop();
bO.dispose();
}}
if(this.__oV){for(bM in this.__oV){var bN=this.__oV[bM];
this.removeListenerById(bN);
}}this.__oS=this.__oT=this.__oU=null;
}});
})();
...
this._setPhase(n);
this.__pg=qx.lang.Function.bind(this._onReadyStateChange,this);
this.__ph=qx.lang.Function.bind(this._onLoad,this);
this.__pi=qx.lang.Function.bind(this._onLoadEnd,this);
this.__pj=qx.lang.Function.bind(this._onAbort,this);
this.__pk=qx.lang.Function.bind(this._onTimeout,this);
this.__pl=qx.lang.Function.bind(this._onError,this);
G.onreadystatechange=this.__pg;
G.onload=this.__ph;
G.onloadend=this.__pi;
G.onabort=this.__pj;
G.ontimeout=this.__pk;
G.onerror=this.__pl;
...
},nullable:true},authentication:{check:B,nullable:true}},members:{__pg:null,__ph:null,__pi:null,__pj:null,__pk:null,__pl:null,__pm:null,__pn:null,__po:null,__pf:null,__pp:null,_transport:null,_createTransport:function(){throw new Error("Abstract method call");
...
}},_getAllRequestHeaders:function(){var Q=qx.lang.Object.merge({},this._getConfiguredRequestHeaders(),this.__pq(),this.__pp,this.__pf);
return Q;
},__pq:function(){var S=this.getAuthentication(),R={};

if(S){S.getAuthHeaders().forEach(function(T){R[T.key]=T.value;
});
return R;
}},setRequestHeader:function(U,V){this.__pf[U]=V;
},getRequestHeader:function(W){return this.__pf[W];
},removeRequestHeader:function(X){if(this.__pf[X]){delete this.__pf[X];
...
},getPhase:function(){return this.__po;
...
},getResponse:function(){return this.__pm;
},_setResponse:function(bb){var bc=bb;

if(this.__pm!==bb){this.__pm=bb;
this.fireEvent(k,qx.event.type.Data,[this.__pm,bc]);
...
if(this.isDone()){this.__pr();
}},__pr:function(){var be;
...
this.fireEvent(bf);
},_setPhase:function(bg){var bh=this.__po;
this.__po=bg;
...
B+=this.__ps(name,z,w);
}}else if(qx.lang.Type.isDate(y)&&x!=null){B+=this.__ps(name,x.format(y),w);
}else{B+=this.__ps(name,y,w);
}}return B.substring(0,B.length-1);
},__ps:function(name,D,E){if(D instanceof qx.core.Object&&E!=null){var F=encodeURIComponent(E(D));
...
},init:true}},members:{__pt:null,_createTransport:function(){return new qx.bom.request.Xhr();
...
if(typeof B.PARSER[A]===o){return this.__pt=B.PARSER[A];
}return this.__pt=A;
},_getParser:function(){var C=this.__pt,D;
if(C){return C;
...
qx.Bootstrap.define(p,{construct:function(){this.__pu=qx.Bootstrap.bind(this.__pG,this);
this.__pv=qx.Bootstrap.bind(this.__pF,this);
this.__pk=qx.Bootstrap.bind(this.__pK,this);
this.__pE();
if(window.attachEvent){this.__pw=qx.Bootstrap.bind(this.__pN,this);
window.attachEvent(a,this.__pw);
}},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{readyState:0,responseText:i,responseXML:null,status:0,statusText:i,timeout:0,open:function(x,y,z,A,B){this.__pP();
...
if(!this.__pO()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();
this.__pE();
}this.__pC.onreadystatechange=this.__pu;

try{if(qx.core.Environment.get(j)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Open native request with method: "+x+", url: "+y+", async: "+z);
}this.__pC.open(x,y,z,A,B);
}catch(C){if(!qx.util.Request.isCrossDomain(y)){throw C;
}
if(!this.__pA){this.__pB=C;
}
if(this.__pA){if(window.XDomainRequest){this.readyState=4;
this.__pC=new XDomainRequest();
this.__pC.onerror=qx.Bootstrap.bind(function(){this.onreadystatechange();
this.onerror();
this.onloadend();
},this);
...
return;
}window.setTimeout(qx.Bootstrap.bind(function(){if(this.__of){return;
}this.readyState=4;
this.onreadystatechange();
this.onerror();
this.onloadend();
},this));
}}if(qx.core.Environment.get(k)===d&&parseInt(qx.core.Environment.get(b),10)<2&&!this.__pA){this.readyState=qx.bom.request.Xhr.OPENED;
this.onreadystatechange();
}},setRequestHeader:function(D,E){this.__pP();
if(D==s||D==t||D==v||D==n){this.__py=true;
}this.__pC.setRequestHeader(D,E);
},send:function(F){this.__pP();
if(!this.__pA&&this.__pB){throw this.__pB;
}if(qx.core.Environment.get(k)===f&&this.timeout===0){this.timeout=10000;
}if(this.timeout>0){this.__pD=window.setTimeout(this.__pk,this.timeout);
}F=typeof F==h?null:F;
try{if(qx.core.Environment.get(j)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Send native request");
}this.__pC.send(F);
}catch(H){if(!this.__pA){throw H;
}if(this._getProtocol()===c){this.readyState=2;
this.__pH();
var G=this;
window.setTimeout(function(){if(G.__of){return;
}G.readyState=3;
G.__pH();
G.readyState=4;
G.__pH();
});
}}if(qx.core.Environment.get(k)===d&&!this.__pA){this.__pG();
}this.__px=true;
},abort:function(){this.__pP();
this.__pn=true;
this.__pC.abort();

if(this.__pC){this.readyState=this.__pC.readyState;
}},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(I){this.__pP();
return this.__pC.getResponseHeader(I);
},getAllResponseHeaders:function(){this.__pP();
return this.__pC.getAllResponseHeaders();
},getRequest:function(){return this.__pC;
},dispose:function(){if(this.__of){return false;
}window.clearTimeout(this.__pD);
if(window.detachEvent){window.detachEvent(a,this.__pw);
}try{this.__pC.onreadystatechange;
}catch(K){return;
}var J=function(){};
this.__pC.onreadystatechange=J;
this.__pC.onload=J;
this.__pC.onerror=J;
this.abort();
this.__pC=null;
...
},_getProtocol:function(){var M=this.__pz;
var N=/^(\w+:)\/\//;
if(M!==null&&M.match){var O=M.match(N);

if(O&&O[1]){return O[1];
}}return window.location.protocol;
},__pC:null,__pA:null,__pu:null,__pv:null,__pw:null,__pk:null,__px:null,__pz:null,__pn:null,__eH:null,__of:null,__pD:null,__pB:null,__py:null,__pE:function(){this.__pC=this._createNativeXhr();
this.__pC.onreadystatechange=this.__pu;
if(this.__pC.onabort){this.__pC.onabort=this.__pv;
}this.__of=this.__px=this.__pn=false;
},__pF:function(){if(!this.__pn){this.abort();
}},__pG:function(){var P=this.__pC,Q=true;
...
if(this.readyState===qx.bom.request.Xhr.DONE&&this.__pn&&!this.__px){return;
}if(!this.__pA&&(P.readyState==2||P.readyState==3)){return;
}this.status=0;
...
}}},__pH:function(){var S=this;
if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__pD);
}if(qx.core.Environment.get(k)==g&&qx.core.Environment.get(b)<8){if(this.__pA&&!this.__px&&this.readyState>=qx.bom.request.Xhr.LOADING){if(this.readyState==qx.bom.request.Xhr.LOADING){return ;
}
if(this.readyState==qx.bom.request.Xhr.DONE){window.setTimeout(function(){if(S.__of){return;
}S.readyState=3;
...
if(this.readyState===qx.bom.request.Xhr.DONE){this.__pI();
}},__pI:function(){if(this.__eH){this.ontimeout();
if(qx.core.Environment.get(k)===f){this.onerror();
}this.__eH=false;
}else{if(this.__pn){this.onabort();
}else{this.__pJ()?this.onerror():this.onload();
}}this.onloadend();
},__pJ:function(){var T;
if(this._getProtocol()===c){T=!this.responseText;
}else{T=!this.statusText;
}return T;
},__pK:function(){var U=this.__pC;
...
this.responseXML=null;
this.__pH();
},__pL:function(){var V=this.readyState===qx.bom.request.Xhr.DONE;
if(this._getProtocol()===c&&this.status===0&&V){if(!this.__pJ()){this.status=200;
}}if(this.status===1223){this.status=204;
}if(qx.core.Environment.get(k)===f){if(V&&this.__py&&!this.__pn&&this.status===0){this.status=304;
}}},__pM:function(){if(qx.core.Environment.get(k)==g&&(this.getResponseHeader(l)||i).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var W=new window.ActiveXObject(o);
W.async=false;
W.validateOnParse=false;
W.loadXML(this.responseText);
this.responseXML=W;
}},__pN:function(){try{if(this){this.dispose();
}}catch(e){}},__pO:function(){var name=qx.core.Environment.get(k);
var X=qx.core.Environment.get(q);
return !(name==g&&X<9||name==d&&X<3.5);
},__pP:function(){if(this.__of){throw new Error("Already disposed");
...
qx.Class.define(f,{extend:tree.ui.popup.Base,construct:function(h){this.__pQ=h;
tree.ui.popup.Base.call(this,this.tr(g));
this.setShowClose(true);
},members:{__pQ:null,_constructInterface:function(){if(this.__pQ===null||this.__pQ.trim()==e){this.__pQ=this.tr(a);
}this.add(new qx.ui.basic.Label(this.__pQ));
...
(function(){var m='/tree/node/{id}/',l="GET",k="POST",j='/tree/getChildrenOf/{id}/',i='/tree/getRoot/',h='Content-Type',g='application/json',f='moveToNode',e='/tree/node/',d="PUT",a='/tree/movenode/',c="DELETE",b="tree.io.rest.Node";
qx.Class.define(b,{extend:tree.io.rest.Resource,construct:function(){var n={"create":{method:k,url:e},"update":{method:d,url:m},"del":{method:c,url:m},"getRoot":{method:l,url:i},"getChildren":{method:l,url:j},"moveToNode":{method:k,url:a}};
tree.io.rest.Resource.call(this,n);
this.configureRequest(function(o,p,q,r){if(p==f){o.setRequestHeader(h,g);
}});
}});
})();
...
},__pT:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__pU:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__pU,this);
return;
...
if(this.__pR){bo.cancelOpen(this.__pR);
this.__pR=null;
...
}if(this.__pR){br.cancelOpen(this.__pR);
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="blur",o="Enter",n="__pW",m="__jG",l="Up",d="__pV",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
}this.__pV=new qx.event.Timer;
this.__pV.addListener(t,this._onOpenInterval,this);
this.__pW=new qx.event.Timer;
this.__pW.addListener(t,this._onCloseInterval,this);
},members:{__pX:null,__pY:null,__pV:null,__pW:null,__jG:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
if(L.isVisible()){if(this.__pX){this.cancelOpen(this.__pX);
}}else if(this.__pX!=L){this.__pX=L;
this.__pV.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__pY){this.cancelClose(this.__pY);
}}else if(this.__pY!=M){this.__pY=M;
this.__pW.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__pX==N){this.__pV.stop();
this.__pX=null;
}},cancelClose:function(O){if(this.__pY==O){this.__pW.stop();
this.__pY=null;
}},_onOpenInterval:function(e){this.__pV.stop();
this.__pX.open();
this.__pX=null;
},_onCloseInterval:function(e){this.__pW.stop();
this.__pY.exclude();
this.__pY=null;
...
}},__qa:{"Enter":1,"Space":1},__mN:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__mN[R]||(this.__qa[R]&&Q.getSelectedButton())){e.stopPropagation();
...
}},__qb:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
}else if(this.__qc.length>0){do{var M=this.__qc[0];
...
var K=false;
if(this.__qc.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__qe(M);
B+=F;
if(C&&this.__qc.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__qc.length>0);
}},__qe:function(N){N.setVisibility(w);
this.__qc.shift();
this.fireDataEvent(p,N);
},__qf:function(O){if(!O){return;
}this.__qc.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__qd.length-1;i>=0;i--){var P=this.__qd[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__qd[T]!=undefined){throw new Error("Priority already in use!");
}this.__qd[T]=S;
...
}for(var i=0;i<this.__qc.length;i++){this.__qc[i].setVisibility(w);
}this.__qc=[];
...
}},__qg:false,_setAllowMenuOpenHover:function(bc){this.__qg=bc;
...
s.addListener(b,this.__qh,this);
this._add(s);
break;
...
},__qh:function(){var w=this.getChildrenContainer().getChildren();
...
},properties:{valid:{check:n,init:false,event:h},text:{init:u,event:j},fieldNameValue:{init:u,event:l}},events:{'continue':i},members:{__qi:null,_constructInterface:function(){var z=new qx.ui.basic.Label(u);
this.add(z);
this.bind(d,z,r);
...
y.addListener(g,this.__qj,this);
y.focus();
this.bind(k,y,r);
...
this.bind(m,w,p);
},__qj:function(){var A=false;

if(this.__qi.getValue().trim().length>0){A=true;
}this.setValid(A);
}}});
})();
...
if(bk&&this.__qk){this.__qp().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__qk){this.__qp().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__qk){if(bs){this._showPlaceholder();
...
if(this.__qk){this.__qp().setStyles(bx);
}if(bv){this.__qn=qx.bom.Label.getTextSize(d,bx);
...
var bC=true;
this.__ql=false;
if(this.__nJ&&this.__nJ===bD){bC=false;
...
}if(bC){this.fireDataEvent(be,bD,this.__nJ);
this.__nJ=bD;
if(this.getLiveUpdate()){this.__qo(bD);
}}},_onWebFontStatusChange:function(bF){if(bF.getData().valid===true){var bG=this.__jK.getStyles();
this.__qn=qx.bom.Label.getTextSize(d,bG);
qx.ui.core.queue.Layout.add(this);
}},__qo:function(bH){var bI=this.__nI;
this.__nI=bH;

if(bI!=bH){this.fireNonBubblingEvent(o,qx.event.type.Data,[bH,bI]);
}},setValue:function(bJ){if(bJ===null){if(this.__ql){return bJ;
}bJ=j;
this.__ql=true;
}else{this.__ql=false;
if(this.__qk){this._removePlaceholder();
}}
...
bL.setValue(bJ);
var bK=this.__ql?null:bJ;
this.__nI=bM;
this.__qo(bK);
}if(this.__qk){this._showPlaceholder();
}return bJ;
}throw new Error("Invalid value type: "+bJ);
},getValue:function(){var bN=this.getContentElement().getValue();
return this.__ql?null:bN;
},resetValue:function(){this.setValue(null);
},_onChangeContent:function(e){this.__ql=e.getData()===null;
this.__qo(e.getData());
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__qp().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__qp().setStyle(bc,L);
}},__qp:function(){if(this.__qm==null){this.__qm=new qx.html.Label();
var bS=qx.theme.manager.Color.getInstance();
this.__qm.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bS.resolve(I)});
this.getContainerElement().add(this.__qm);
}return this.__qm;
},_onChangeLocale:qx.core.Environment.select(g,{"true":function(e){var content=this.getPlaceholder();

if(content&&content.translate){this.setPlaceholder(content.translate());
}},"false":null}),_applyPlaceholder:function(bT,bU){if(this.__qk){this.__qp().setValue(bT);
...
}}},destruct:function(){this.__qm=this.__jK=null;
...
this.__qq=o;
},members:{__qq:null,__qr:null,__qs:null,_createDomElement:function(){return qx.bom.Input.create(this.__qq);
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__qs=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__qr?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__qr=w;
qx.html.Element.prototype.setSelectable.call(this,this.__qs&&w);
...
},setWrap:function(B,C){if(this.__qq===l){this._setProperty(n,B,C);
}else{throw new Error("Text wrapping is only support by textareas!");
}return this;
},getWrap:function(){if(this.__qq===l){return this._getProperty(n);
}else{throw new Error("Text wrapping is only support by textareas!");
}}}});
})();
...
(function(){var m='undefined',k="",j='</span></div></div>',h='.',g='<div class="wrapper" id="preview-',f='body',d='unselectable',c="html/cells",b="data-item-coordinates='",a="error",bh='"></div><div class="label"><span>',bg='updated',bf="default",be="data-item-coordinates",bd='changeState',bc='_applyItems',bb=',',ba="auto",Y='success',X='loadend',t="'",u='" height="',r='"><canvas width=20 height=20></canvas><div class="img-wrapper"><img src="" height="" width=""></div><div class="label"><span>',s="dragexit",p='changeNode',q='complete',n="move",o='_applyNode',x="drop",y="layer",G="abort",E="resize",N='running',I='gallery',T="dragstart",R="multi",A='appear',W='"><canvas width=20 height=20></canvas><div class="img-wrapper"><img src="',V="dragenter",U="off",z='" width="',C=' ',D='thumbnail ',F='',H="manager",J="droprequest",O='#preview-',S="position: absolute;",v='thumbnail',w="tree.ui.gallery.Gallery",B="dragend",M="dragover",L='thumbnail-selected',K="text-align: center;",Q='mouseup',P=",";
qx.Class.define(w,{extend:qx.ui.container.Composite,properties:{items:{init:null,apply:bc},node:{init:null,nullable:true,apply:o,event:p}},construct:function(bi){qx.ui.container.Composite.call(this,bi);
this.__qt={};
this.addListener(Q,this._onMouseUp,this);
this.addListenerOnce(A,function(){var bl=this.getContainerElement().getDomElement();
bl.addEventListener(x,qx.lang.Function.bind(this._onDrop,this));
bl.addEventListener(V,this._noopHandler,false);
bl.addEventListener(s,this._noopHandler,false);
bl.addEventListener(M,this._noopHandler,false);
},this);
this.setLayout(new qx.ui.layout.Grow());
...
this.itemWidth=110;
this.itemCount=0;
var bj=this._createScroller();
this.__qu=bj;
bj.set({scrollbarX:U,scrollbarY:ba});
this.add(bj);
bj.getPane().addListener(E,this._onPaneResize,this);
this.manager=new qx.ui.virtual.selection.CellRectangle(bj.getPane(),this).set({mode:R,drag:false});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bj);
var bk=qx.theme.manager.Font.getInstance().resolve(bf).getStyles();
this._fontCss=qx.bom.element.Style.compile(bk);
this.initItems();
this.setDraggable(true);
this.addListener(T,function(e){var bm=this.manager.getSelection();

for(var i=0,l=bm.length;i<l;i++){if(typeof (this.getItemData(bm[i].row,bm[i].column).uploadId)!==m){e.preventDefault();
return;
}}e.addAction(n);
e.addType(c);
qx.bom.Collection.query(f).addClass(d);
});
this.addListener(B,function(e){qx.bom.Collection.query(f).removeClass(d);
});
this.addListener(J,function(e){var bo=e.getCurrentType();
var bp=[];

if(bo===c){var bn=this.manager.getSelection();

for(var i=0,l=bn.length;i<l;i++){bp.push(this.getItemData(bn[i].row,bn[i].column));
}}e.addData(bo,bp);
},this);
},members:{__qu:null,__qt:null,_thumbnailPreview:function(bq,br,bs,event){var bt=new Image();
bt.src=bq.result;
bt.onload=qx.lang.Function.bind(function(){var bx=bt.height;
var bu=bt.width;
bt=null;

if(bx>82){var bv=bx/82;
bx=parseInt(bx/bv);
bu=parseInt(bu/bv);
}
if(bu>90){var bv=bu/90;
bx=parseInt(bx/bv);
bu=parseInt(bu/bv);
}var bw=g+bs+W+event.target.result+u+bx+z+bu+bh+this._getTitleForFile(br)+j;
this._addPreview(bs,bw);
},this);
},_getTitleForFile:function(by){var bz=by.name.split(h);
bz.splice(-1,1);
return bz.join(h);
},_noThumbnailPreview:function(bA,bB){var bC=g+bB+r+this._getTitleForFile(bA)+j;
this._addPreview(bB,bC);
},_addPreview:function(bD,bE){var bF={html:bE,uploadId:bD};
this.__qt[this.getNode().getPk()][bD].item=bF;
this.getItems().push(bF);
this.updateItems();
},_noopHandler:function(bG){bG.stopPropagation();
bG.preventDefault();
},_onDrop:function(e){e.stopPropagation();
e.preventDefault();
var bN=e.dataTransfer;
var bH=bN.files;
var bJ=0;
var bK=1*1024*1024;

for(var i=0;i<bH.length;i++){var bL=bH[i];
var bM=Math.ceil(Math.random()*10000).toString();
var bI=new tree.io.Uploader(bL,{node:this.getNode().getPk()});

if(typeof (this.__qt[this.getNode().getPk()])==m){this.__qt[this.getNode().getPk()]={};
}this.__qt[this.getNode().getPk()][bM]={uploader:bI,item:null,progress:new tree.ui.Progress(bI)};
bJ+=bL.size;

if(bJ>bK){this._noThumbnailPreview(bL,bM);
}else{var bO=new FileReader();
if(bO.addEventListener){bO.addEventListener(X,qx.lang.Function.bind(this._thumbnailPreview,this,bO,bL,bM),false);
}else{bO.onloadend=qx.lang.Function.bind(this._thumbnailPreview,this,bO,bL,bM);
}bO.readAsDataURL(bL);
}bI.addListener(bd,qx.lang.Function.bind(function(bP,bQ,bR,event){var bS=event.getData();

if(bS==q){qx.util.TimerManager.getInstance().start(function(){var bU=bR.getItem();
var bV=this.getItems();
var bT=false;

for(var i=0,l=bV.length;i<l;i++){if(bV[i].uploadId==bQ){bV.splice(i,1,bU);
bT=true;
break;
}}this.__qv(bP,bQ);

if(bT===true){this.updateItems();
}},0,this,null,1000);
}else if([a,G].indexOf(bS)!==-1){this.__qv(bP,bQ);
}},this,this.getNode(),bM,bI),this);
bI.start();
}return false;
},__qv:function(bW,bX){this.__qt[bW.getPk()][bX].uploader.dispose();
delete this.__qt[bW.getPk()][bX];
},updateItems:function(){var cc=this.getItems();
this.itemCount=cc!==null?cc.length:0;
var cb=this.__qu.getPane();
var bY=this.__qu.getPaneSize()!==null?this.__qu.getPaneSize().width:this.getWidth();
var ca=Math.floor(bY/this.itemWidth);
this.itemPerLine=ca;
var cd=Math.ceil(this.itemCount/ca);
cb.getColumnConfig().setItemCount(ca);
cb.getRowConfig().setItemCount(cd);
cb.fullUpdate();
},_applyItems:function(ce,cf){if(this.getNode()!==null){var cg=this.__qt[this.getNode().getPk()];

if(typeof (cg)!==m){for(var ch in cg){if(cg[ch].uploader.getState()==N){this.getItems().push(cg[ch].item);
}}}}this.manager.clearSelection();
this.updateItems();
},_applyNode:function(ci,cj){if(cj!==null){var ck=this.__qt[cj.getPk()];

if(typeof (ck)!==m){for(var cl in ck){ck[cl].progress.dispose();
ck[cl].progress=null;
}}}
if(ci!==null){this.refreshNodeItems();
var ck=this.__qt[ci.getPk()];

if(typeof (ck)!==m){for(cl in ck){var cm=this.__qt[this.getNode().getPk()][cl];
cm.progress=new tree.ui.Progress(cm.uploader);
}}}},refreshNodeItems:function(){var cn=new tree.io.rest.Item();
cn.addListener(Y,function(event){this.setItems(event.getData());
},this);
cn.getForNode({id:this.getNode().getPk()});
},_createScroller:function(){var co=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new tree.ui.virtual.layer.HtmlCell(this);
co.getPane().addLayer(this.layer);
this.layer.addListener(bg,this._onLayerUpdated,this);
return co;
},getItemData:function(cp,cq){return this.getItems()[cp*this.itemPerLine+cq];
},isItemSelectable:function(cr){return !!this.getItemData(cr.row,cr.column);
},_onMouseUp:function(event){if(typeof (on_item_click)===m){return;
}var cs=event.getNativeEvent();

if(cs.altKey===true||cs.ctrlKey===true||cs.metaKey===true||cs.shiftKey===true){return;
}var cv=qx.bom.Collection.create(event.getOriginalTarget());

if(cv.getClass().split(C).indexOf(v)!==-1){var cu=cv.getAttribute(be).split(bb);
var ct=this.getItemData(parseInt(cu[0]),parseInt(cu[1]));

if(typeof (ct.uploadId)===m){on_item_click(ct);
}}},_onPaneResize:function(e){var cy=e.getTarget();
var cw=e.getData().width;
var cx=Math.floor(cw/this.itemWidth);

if(cx==this.itemsPerLine){return;
}this.itemPerLine=cx;
var cz=Math.ceil(this.itemCount/cx);
cy.getColumnConfig().setItemCount(cx);
cy.getRowConfig().setItemCount(cz);
},_onLayerUpdated:function(){if(this.getNode()!==null){var cA=this.__qt[this.getNode().getPk()];

if(typeof (cA)!==m){for(var cB in cA){var cC=qx.bom.Collection.query(O+cB);

if(cC.length==1){cA[cB].progress.setElement(cC);
}}}}},styleSelectable:function(cD,cE,cF){this.layer.updateLayerData();
},getCellProperties:function(cG,cH){var cK=this.getItemData(cG,cH);

if(!cK){return k;
}var cJ=this.manager.isItemSelected({row:cG,column:cH});
var cI=k;
return {style:[S,K,this._fontCss,cI].join(k),classes:D+(cJ?L:F),content:cK.html,attributes:b+cG+P+cH+t};
...
this.__qw=r;
this.__qx=r.bind(q,this,q);
this.__pD=qx.util.TimerManager.getInstance().start(this._update,150,this);
r.addListener(h,this._onUploaderChangeState,this);
},members:{__qy:0,__pD:null,__qw:null,__qx:null,_onUploaderChangeState:function(event){switch(event.getData()){case c:break;
case n:break;
case b:break;
}},_update:function(){this.__qy++;
this.__qz();
},_applyElement:function(s,t){this.__qz();
},_applyPercentComplete:function(u,v){this.__qz();
},__qz:function(){if(this.getElement()===null){return;
...
this.__qw.removeBinding(this.__qx);
}});
})();
(function(){var c="interval",b="qx.util.TimerManager",a="singleton";
qx.Class.define(b,{extend:qx.core.Object,type:a,statics:{__qA:[],__qB:{},__pD:0},members:{__qC:false,start:function(d,e,f,g,h){if(!h){h=e||0;
}var j=(new Date()).getTime()+h;
this.self(arguments).__qB[++this.self(arguments).__pD]={callback:d,userData:g||null,expireAt:j,recurTime:e,context:f||this};
this.__qD(j,this.self(arguments).__pD);
return this.self(arguments).__pD;
},stop:function(k){var l=this.self(arguments).__qA;
var length=l.length;

for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);
break;
}}delete this.self(arguments).__qB[k];
if(l.length==0&&this.__qC){qx.event.Idle.getInstance().removeListener(c,this.__qE,this);
this.__qC=false;
}},__qD:function(m,n){var p=this.self(arguments).__qA;
var o=this.self(arguments).__qB;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(!this.__qC){qx.event.Idle.getInstance().addListener(c,this.__qE,this);
this.__qC=true;
}},__qE:function(){var t=(new Date()).getTime();
var r=this.self(arguments).__qA;
var s=this.self(arguments).__qB;
...
this.__qD(q.expireAt,v);
}else{delete s[v];
}}if(r.length==0&&this.__qC){qx.event.Idle.getInstance().removeListener(c,this.__qE,this);
this.__qC=false;
}}}});
})();
...
}},members:{__qF:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qF;
},getMarkup:function(e){if(this.__qF){return this.__qF;
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__qF:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__qF;
},_applyStyle:function(){},getMarkup:function(){if(this.__qF){return this.__qF;
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__qG=new qx.ui.decoration.css3.BorderImage();

if(p){this.__qH(p);
}}else{this.__qG=new qx.ui.decoration.GridDiv(p);
}
if(q!=null){this.__qG.setInsets(q);
}},properties:{baseImage:{check:d,nullable:true,apply:j},insetLeft:{check:o,nullable:true,apply:n},insetRight:{check:o,nullable:true,apply:n},insetBottom:{check:o,nullable:true,apply:n},insetTop:{check:o,nullable:true,apply:n},insets:{group:[k,l,i,e],mode:f}},members:{__qG:null,getMarkup:function(){return this.__qG.getMarkup();
},resize:function(r,s,t){this.__qG.resize(r,s,t);
},tint:function(u,v){},getInsets:function(){return this.__qG.getInsets();
},_applyInsets:function(w,x,name){var y=g+qx.lang.String.firstUp(name);
this.__qG[y](w);
},_applyBaseImage:function(z,A){if(this.__qG instanceof qx.ui.decoration.GridDiv){this.__qG.setBaseImage(z);
}else{this.__qH(z);
}},__qH:function(B){var G,H,J,I;
this.__qG.setBorderImage(B);
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__qF:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qF;
...
this.__qF=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__qF;
...
qx.Mixin.define(o,{include:[qx.ui.decoration.MSingleBorder,qx.ui.decoration.MBackgroundImage],construct:function(){this._getDefaultInsetsForBorder=this.__qM;
this._resizeBorder=this.__qL;
this._styleBorder=this.__qJ;
this._generateMarkup=this.__qK;
},properties:{innerWidthTop:{check:f,init:0},innerWidthRight:{check:f,init:0},innerWidthBottom:{check:f,init:0},innerWidthLeft:{check:f,init:0},innerWidth:{group:[w,z,n,k],mode:a},innerColorTop:{nullable:true,check:g},innerColorRight:{nullable:true,check:g},innerColorBottom:{nullable:true,check:g},innerColorLeft:{nullable:true,check:g},innerColor:{group:[u,v,x,r],mode:a}},members:{__qI:null,__qJ:function(D){{var F=qx.theme.manager.Color.getInstance();
...
}},__qK:function(K){var O=this._generateBackgroundMarkup(K);
...
K[y]=0;
K[p]=0;
return this.__qI=this._generateBackgroundMarkup(K,O);
},__qL:function(S,T,U){var V=this.getInsets();
T-=V.left+V.right;
U-=V.top+V.bottom;
...
},__qM:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
