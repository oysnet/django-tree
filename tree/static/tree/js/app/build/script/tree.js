...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:tree.15f3f6485564.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l='/',k='tree/js/app/build/resource',j='selection[0]',i="horizontal",h='itemsloaded',g='dropend',f="tree.Application",e='path',d="tree-container",c='node',a="/",b='undefined';
qx.Class.define(f,{extend:qx.application.Inline,members:{main:function(){if(typeof (STATIC_URL)!==b){var o=STATIC_URL;

if(o.substr(-1)!==l){o+=l;
}qx.$$libraries.qx.resourceUri=o+k;
qx.$$libraries.tree.resourceUri=o+k;
}qx.application.Inline.prototype.main.call(this);
var m=document.getElementById(d);
var n=new qx.ui.root.Inline(m,true,true);
n.setLayout(new qx.ui.layout.Grow);
var q=new qx.ui.splitpane.Pane(i);
n.add(q);
var r=getQueryVariable(e),p=null;

if(r!=null){r=r.split(l);
p=r.pop();
r=r.join(a);
}var t=new tree.ui.tree.Tree(r);
var s=new tree.ui.gallery.Gallery(null,t);

if(p!==null){s.addListenerOnce(h,function(){s.selectItem(p);
});
}q.add(t);
q.add(s,5);
t.bind(j,s,c);
t.addListener(g,s.refreshNodeItems,s);
}}});
})();
...
if(bj&&!parent.getLoaded()){this.__nB(parent);
}return bj;
},this)},be,bf);
},this)};
this.setDelegate(P);
this.__nv();
},members:{__ns:null,__nr:null,__nu:null,__nt:true,__nv:function(){var bn=new qx.ui.menu.Menu();
var bl=new qx.ui.menu.Button(this.tr(E),I);
bn.add(bl);
bl.addListener(h,this.__ny,this);
var bm=new qx.ui.menu.Button(this.tr(d),t);
bn.add(bm);
bm.addListener(h,this.__nx,this);
var bk=new qx.ui.menu.Button(this.tr(K),M);
this.__nu=bk;
bn.add(bk);
bk.addListener(h,this.__nw,this);
this.addListener(y,function(){if(this.getSelection().getItem(0).getChildren().getLength()>0){this.__nu.setEnabled(false);
}else{this.__nu.setEnabled(true);
}},this);
this.setContextMenu(bn);
},__nw:function(){var bo=this.getSelection().getItem(0);
...
},__nx:function(){var bt=this.getSelection().getItem(0);
...
},__ny:function(){var by=new tree.ui.popup.EditCreate();
...
},__nz:function(bD){for(var i=0,l=bD.getLength();i<l;i++){var n=bD.getItem(i);
this.__ns[n.getPk()]=n;

if(n.getChildren().getLength()>0&&n.getLoaded()===true){this.__nz(n.getChildren());
}}},__nA:function(parent,bE){var bF=new tree.io.rest.Node();
...
bF.loadPath({path:bE});
},__nB:function(parent,bK){bK=typeof (bK)!==o?bK:false;
...
if(this.__nt){this.__nt=false;

if(bM.getLength()>0){this.__nB(bM.getItem(0),false);
}}},this,parent));
...
}},members:{__nC:null,__nD:function(n){return qx.Bootstrap.getKeys(n).sort().join(e);
...
}if(this.__nC&&this.__nC.getModelClass&&this.__nC.getModelClass(r)!=null){return;
}var y={};
var x={__nE:this.__nE};
...
if(p){y[s].apply=a;
}if(this.__nC&&this.__nC.getValidationRule){var u=this.__nC.getValidationRule(r,s);

if(u){y[s].validate=f+s;
x[f+s]=u;
}}}if(this.__nC&&this.__nC.getModelSuperClass){var w=this.__nC.getModelSuperClass(r)||qx.core.Object;
}else{var w=qx.core.Object;
}var t=[];

if(this.__nC&&this.__nC.getModelMixins){var v=this.__nC.getModelMixins(r);
if(!qx.lang.Type.isArray(v)){if(v!=null){t=[v];
}}}if(p){t.push(qx.data.marshal.MEventBubbling);
}var q={extend:w,include:t,properties:y,members:x,destruct:this.__nF};
qx.Class.define(j+r,q);
},__nF:function(){var z=qx.util.PropertyUtil.getAllProperties(this.constructor);

for(var A in z){this.__nE(this.get(z[A].name));
}},__nE:function(B){if(!(B instanceof qx.core.Object)){return ;
}if(B.isDisposed()){return;
}B.dispose();
},__nG:function(C){var D;
if(this.__nC&&this.__nC.getModelClass){D=this.__nC.getModelClass(C);
}
...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__nJ:false,__nK:null,__nL:null,__nM:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===g)){return true;
}
if(C===j&&(D===m||D===g||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&(qx.core.Environment.get(d)<9||(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)<9))){if(!E.__nN){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__nO:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(d)<9&&qx.core.Environment.get(A)<9){if(O.__nN){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__nP:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__nL){this.__nL=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__nM){this.__nM=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__nJ=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__nJ=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__nK&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__nK);
...
if(!this.__nJ||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__nK=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
if(bf&&bf.nodeType){return this.__nU([bf]);
}else{return bf;
}}return null;
};
};
...
},__nQ:/^[^<]*(<(.|\s)+>)[^>]*$|^#([\w-]+)$/,create:function(bp,bq){var bs=qx.bom.Collection;
if(bp.nodeType){return new bs(bp);
}else if(typeof bp===I){var br=bs.__nQ.exec(bp);
...
},__nS:/^.[^:#\[\.,]*$/,not:function(bI){if(this.__nS.test(bI)){var bJ=qx.bom.Selector.matches(g+bI+d,this);
return this.__nU(bJ);
...
return this.__nU(qx.lang.Array.unique(bT));
...
if(this.length===1){return this.__nU(cb.query(bY,this[0]));
}else{var ca=[];

for(var i=0,l=this.length;i<l;i++){ca.push.apply(ca,cb.query(bY,this[i]));
}return this.__nU(qx.lang.Array.unique(ca));
...
},nextAll:function(cf){return this.__nT(y,cf);
...
},prevAll:function(cj){return this.__nT(c,cj);
...
},parents:function(cm){return this.__nT(H,cm);
},siblings:function(cn){return this.__nT(s,cn);
},__nT:function(co,cp){var cr=[];
...
},__nU:function(ct){var cu=new qx.bom.Collection;
cu.__nR=this;
ct=Array.prototype.slice.call(ct,0);
cu.push.apply(cu,ct);
return cu;
},andSelf:function(){return this.add(this.__nR);
},end:function(){return this.__nR||new qx.bom.Collection();
},__nV:function(cv,cw){var cB=this[0];
...
},__nW:function(cG,cH){var cJ=qx.bom.Selector;
...
},append:function(cM){return this.__nV(arguments,this.__nX);
},prepend:function(cN){return this.__nV(arguments,this.__nY);
},__nX:function(cO,cP){cO.appendChild(cP);
},__nY:function(cQ,cR){cQ.insertBefore(cR,cQ.firstChild);
},appendTo:function(cS){return this.__nW(arguments,e);
},prependTo:function(cT){return this.__nW(arguments,J);
},before:function(cU){return this.__nV(arguments,this.__oa);
},after:function(cV){return this.__nV(arguments,this.__ob);
},__oa:function(cW,cX){cW.parentNode.insertBefore(cX,cW);
},__ob:function(cY,da){cY.parentNode.insertBefore(da,cY.nextSibling);
},insertBefore:function(db){return this.__nW(arguments,w);
},insertAfter:function(dc){return this.__nW(arguments,q);
...
}dd.map(this.__oc).append(this);
}return this;
},__oc:function(df){while(df.firstChild){df=df.firstChild;
}return df;
...
},replaceAll:function(di){return this.__nW(arguments,n);
...
qx.Class.define(n,{statics:{__od:function(K,L,M){return M.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)$/i)?K:L+z+M+H;
},__oe:{opt:[1,e,c],leg:[1,f,u],table:[1,h,y],tr:[2,r,E],td:[3,I,b],col:[2,G,C],def:qx.core.Environment.select(g,{"mshtml":[1,s,d],"default":null})},__of:function(N,O){var U=O.createElement(J);
N=N.replace(/(<(\w+)[^>]*?)\/>/g,this.__od);
...
if(typeof ba===m){ba=this.__of(ba,X);
...
qx.Bootstrap.define(d,{construct:function(){this.__og=qx.Bootstrap.bind(this.__oj,this);
this.__kH=document.createElement(f);
},statics:{TIMEOUT:15},members:{__oh:null,__oi:null,__bD:null,__bE:null,__og:null,__kH:null,load:function(q,r,s){if(this.__oh){throw new Error("Another request is still running!");
}this.__oh=true;
this.__oi=false;
...
this.__bE=s||window;
u.type=b;
u.onerror=u.onload=u.onreadystatechange=this.__og;
var self=this;
...
},abort:function(){if(this.__oh){this.dispose(a);
}},dispose:function(status){if(this.__oi){return;
}this.__oi=true;
var x=this.__kH;
...
},__oj:qx.event.GlobalError.observeMethod(function(e){var y=qx.bom.client.Engine.getName();
...
(function(){var b="__ok",a="qx.ui.window.Manager";
qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__ok:null,setDesktop:function(c){this.__ok=c;
this.updateStack();
},getDesktop:function(){return this.__ok;
},changeActiveWindow:function(d,e){if(d){this.bringToFront(d);
d.setActive(true);
}
if(e){e.resetActive();
}},_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this.__ok.forceUnblockContent();
var f=this.__ok.getWindows();
...
}}this.__ok.setActiveWindow(k);
},bringToFront:function(n){var o=this.__ok.getWindows();
var p=qx.lang.Array.remove(o,n);

if(p){o.push(n);
this.updateStack();
}},sendToBack:function(q){var r=this.__ok.getWindows();
...
qx.Mixin.define(b,{members:{__ol:function(m,n,o,p){var q=this.getChildrenContainer();
...
(function(){var l="move",k="Boolean",j="__on",i="mouseup",h="mousedown",g="losecapture",f="qx.ui.core.MMovable",d="mousemove",c="__om",b="maximized",a="move-frame";
qx.Mixin.define(f,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__om:null,__on:null,__oo:null,__op:null,__oq:null,__or:null,__os:null,__ot:false,__ou:null,__ov:0,_activateMoveHandle:function(m){if(this.__om){throw new Error("The move handle could not be redefined!");
}this.__om=m;
...
m.addListener(g,this.__oz,this);
},__ow:function(){var n=this.__on;

if(!n){n=this.__on=new qx.ui.core.Widget();
n.setAppearance(a);
n.exclude();
qx.core.Init.getApplication().getRoot().add(n);
}return n;
},__ox:function(){var location=this.getContainerLocation();
var p=this.getBounds();
var o=this.__ow();
o.setUserBounds(location.left,location.top,p.width,p.height);
o.show();
o.setZIndex(this.getZIndex()+1);
},__oy:function(e){var r=this.__oo;
var u=Math.max(r.left,Math.min(r.right,e.getDocumentLeft()));
var t=Math.max(r.top,Math.min(r.bottom,e.getDocumentTop()));
var q=this.__op+u;
var s=this.__oq+t;
return {viewportLeft:q,viewportTop:s,parentLeft:q-this.__or,parentTop:s-this.__os};
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__ou=parent.getBlockerColor();
this.__ov=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__ot=true;
}}this.__oo={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__or=w.left;
this.__os=w.top;
this.__op=v.left-e.getDocumentLeft();
this.__oq=v.top-e.getDocumentTop();
this.addState(l);
this.__om.capture();
if(this.getUseMoveFrame()){this.__ox();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__oy(e);

if(this.getUseMoveFrame()){this.__ow().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__ot){parent.unblockContent();
parent.setBlockerColor(this.__ou);
parent.setBlockerOpacity(this.__ov);
this.__ou=null;
this.__ov=0;
this.__ot=false;
}}this.__om.releaseCapture();
var z=this.__oy(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__ow().exclude();
}e.stopPropagation();
},__oz:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__ow().exclude();
}}},destruct:function(){this._disposeObjects(j,c);
this.__oo=null;
}});
})();
...
A.addListener(i,this.__oN,this,true);
A.addListener(q,this.__oO,this);
A.addListener(x,this.__oQ,this);
A.addListener(s,this.__oR,this);
A.addListener(o,this.__oP,this);
var B=this.getContainerElement().getDomElement();

if(B==null){B=window;
}this.__oA=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);
},properties:{resizableTop:{check:k,init:true},resizableRight:{check:k,init:true},resizableBottom:{check:k,init:true},resizableLeft:{check:k,init:true},resizable:{group:[l,d,m,y],mode:v},resizeSensitivity:{check:a,init:5},useResizeFrame:{check:k,init:true}},members:{__oA:null,__oB:null,__oC:null,__oD:null,__oE:null,__oF:null,__oG:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__oB;

if(!C){C=this.__oB=new qx.ui.core.Widget();
C.setAppearance(p);
C.exclude();
qx.core.Init.getApplication().getRoot().add(C);
}return C;
},__oH:function(){var location=this.__oK();
...
},__oI:function(e){var F=this.__oC;
...
if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){L=Math.max(K.top,Math.min(K.bottom,e.getDocumentTop()))-this.__oE;
...
if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){L=Math.max(K.left,Math.min(K.right,e.getDocumentLeft()))-this.__oD;
...
},__oJ:{1:f,2:n,4:h,8:z,5:t,6:g,9:c,10:b},__oK:function(){var O=this.getDecoratorElement();
if(O&&O.getDomElement()){return qx.bom.element.Location.get(O.getDomElement());
}else{return this.getContentLocation();
}},__oL:function(e){var location=this.__oK();
var P=this.getResizeSensitivity();
var S=e.getDocumentLeft();
var R=e.getDocumentTop();
var Q=this.__oM(location,S,R,P);
if(Q>0){Q=Q|this.__oM(location,S,R,P*2);
}this.__oC=Q;
},__oM:function(location,T,U,V){var W=0;
...
},__oN:function(e){if(!this.__oC){return;
}this.addState(j);
this.__oD=e.getDocumentLeft();
this.__oE=e.getDocumentTop();
var bb=this.getContainerLocation();
var X=this.__oK();
var ba=this.getBounds();
this.__oF={top:X.top,left:X.left,containerWidth:bb.right-bb.left,containerHeight:bb.bottom-bb.top,width:X.right-X.left,height:X.bottom-X.top,bounds:qx.lang.Object.clone(ba)};
var parent=this.getLayoutParent();
var bc=parent.getContentLocation();
var Y=parent.getBounds();
this.__oG={left:bc.left,top:bc.top,right:bc.left+Y.width,bottom:bc.top+Y.height};
if(this.getUseResizeFrame()){this.__oH();
}this.capture();
e.stop();
},__oO:function(e){if(!this.hasState(j)){return;
...
this.releaseCapture();
e.stopPropagation();
},__oP:function(e){if(!this.__oC){return;
}this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
this.removeState(w);
if(this.getUseResizeFrame()){this._getResizeFrame().exclude();
}},__oQ:function(e){if(this.hasState(j)){var bh=this.__oI(e);
...
}else if(!this.hasState(u)&&!this.__oA.isSessionActive()){this.__oL(e);
var bi=this.__oC;
var bg=this.getApplicationRoot();

if(bi){var be=this.__oJ[bi];
this.setCursor(be);
bg.setGlobalCursor(be);
}else if(this.getCursor()){this.resetCursor();
bg.resetGlobalCursor();
}}},__oR:function(e){if(this.getCursor()&&!this.hasState(j)){this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
}}},destruct:function(){if(this.__oB!=null&&!qx.core.ObjectRegistry.inShutDown){this.__oB.destroy();
this.__oB=null;
}this.__oA=null;
}});
})();
...
this.__oT=bh.left===undefined?0:bh.left;
this.__oS=bh.top===undefined?0:bh.top;
...
this.__oT=bi.left===undefined?0:bi.left;
this.__oS=bi.top===undefined?0:bi.top;
this.removeState(b);
this.hide();
this.fireEvent(u);
...
},members:{__oU:null,_applyCloseOnEscape:function(h,i){if(h===true){this.__oU=this.addListener(c,this.__fR,this);
}else if(this.__oU!==null){this.removeListenerById(this.__oU);
this.__oU=null;
...
try{if(typeof r!==q){this.__pg(r);
}}catch(e){this.dispose();
throw e;
}},events:{"success":p,"actionSuccess":p,"error":p,"actionError":p},statics:{POLL_THROTTLE_LIMIT:100,POLL_THROTTLE_COUNT:30,REQUIRED:true},members:{__oV:null,__oW:null,__pa:null,__oX:null,__oY:null,__pb:null,configureRequest:function(s){this.__pb=s;
},_getRequest:function(){return new qx.io.request.Xhr();
},__pc:function(t){var u=this._getRequest();

if(!qx.lang.Type.isArray(this.__oV[t])){this.__oV[t]=[];
}this.__oV[t].push(u);
return u;
},map:function(v,w,x,y){this.__oW[v]=[w,x,y];
this.__oV[v]=[];
...
this[v].action=true;
},invoke:function(z,A){var D=this.__pc(z),G=this._getRequestConfig(z,A),A=A==null?{}:A,C=qx.lang.Object.clone(A);
this.__oW[z].params=A;
if(A){this.__pf(this.__oW[z][1]).forEach(function(H){delete C[H];
});
}this.__pd(A,G.check);

if(this.__pb){this.__pb.call(this,D,z,A);
}this.__pe(D,C,z,G);
...
},setBaseUrl:function(K){this.__pa=K;
},__pd:function(L,M){if(typeof M!==q){qx.lang.Object.getKeys(M).forEach(function(N){if(M[N]===qx.io.rest.Resource.REQUIRED&&typeof L[N]===q){throw new Error("Missing parameter '"+N+"'");
}if(!(M[N]&&typeof M[N].test==l)){return;
}if(!M[N].test(L[N])){throw new Error("Parameter '"+N+"' is invalid");
}});
}},__pe:function(O,P,Q,R){O.set({method:R.method,url:R.url});
...
if(this.__oV[U]){Y.forEach(function(ba){ba.abort();
});
}}},refresh:function(bb){this.invoke(bb,this.__oW[bb].params);
},poll:function(bc,bd,be){if(this.__oX[bc]){this.__oX[bc].dispose();
}if(typeof be==q){be=this.__oW[bc].params;
}this.invoke(bc,be);
var bf=this.__oX[bc]=new qx.event.Timer(bd);
bf.addListener(h,function bg(){var bh=this.__oV[bc][0];
...
}var bl=this.__oY[bi]=this.addListener(bi+o,function bp(){if(bk.isDisposed()){return;
}
if(!bj()){bn=new Date();
bk.refresh(bi);
}},bm);
this.invoke(bi);
return bl;
},_getRequestConfig:function(br,bs){var bu=this.__oW[br];

if(!qx.lang.Type.isArray(bu)){throw new Error("No route for action "+br);
}var bw=bu[0],bt=this.__pa!==null?this.__pa+bu[1]:bu[1],bx=bu[2],bv=this.__pf(bt);
bs=bs||{};
...
},__pf:function(bB){var bD=/\{(\w+)(=\w+)?\}/g,bE,bC=[];
while((bE=bD.exec(bB))){bC.push(bE[1]);
}return bC;
},__pg:function(bF){qx.lang.Object.getKeys(bF).forEach(function(bG){var bI=bF[bG],bJ=bI.method,bH=bI.url,bK=bI.check;
this.map(bG,bJ,bH,bK);
},this);
},__ph:function(bL){if(!this.constructor.$$events){this.constructor.$$events={};
}
if(!this.constructor.$$events[bL]){this.constructor.$$events[bL]=p;
}}},destruct:function(){var bM;

for(bM in this.__oV){if(this.__oV[bM]){this.__oV[bM].forEach(function(bP){bP.dispose();
});
}}
if(this.__oX){for(bM in this.__oX){var bO=this.__oX[bM];
bO.stop();
bO.dispose();
}}
if(this.__oY){for(bM in this.__oY){var bN=this.__oY[bM];
this.removeListenerById(bN);
}}this.__oV=this.__oW=this.__oX=null;
}});
})();
...
this._setPhase(n);
this.__pj=qx.lang.Function.bind(this._onReadyStateChange,this);
this.__pk=qx.lang.Function.bind(this._onLoad,this);
this.__pl=qx.lang.Function.bind(this._onLoadEnd,this);
this.__pm=qx.lang.Function.bind(this._onAbort,this);
this.__pn=qx.lang.Function.bind(this._onTimeout,this);
this.__po=qx.lang.Function.bind(this._onError,this);
G.onreadystatechange=this.__pj;
G.onload=this.__pk;
G.onloadend=this.__pl;
G.onabort=this.__pm;
G.ontimeout=this.__pn;
G.onerror=this.__po;
...
},nullable:true},authentication:{check:B,nullable:true}},members:{__pj:null,__pk:null,__pl:null,__pm:null,__pn:null,__po:null,__pp:null,__pq:null,__pr:null,__pi:null,__ps:null,_transport:null,_createTransport:function(){throw new Error("Abstract method call");
...
}},_getAllRequestHeaders:function(){var Q=qx.lang.Object.merge({},this._getConfiguredRequestHeaders(),this.__pt(),this.__ps,this.__pi);
return Q;
},__pt:function(){var S=this.getAuthentication(),R={};

if(S){S.getAuthHeaders().forEach(function(T){R[T.key]=T.value;
});
return R;
}},setRequestHeader:function(U,V){this.__pi[U]=V;
},getRequestHeader:function(W){return this.__pi[W];
},removeRequestHeader:function(X){if(this.__pi[X]){delete this.__pi[X];
...
},getPhase:function(){return this.__pr;
...
},getResponse:function(){return this.__pp;
},_setResponse:function(bb){var bc=bb;

if(this.__pp!==bb){this.__pp=bb;
this.fireEvent(k,qx.event.type.Data,[this.__pp,bc]);
...
if(this.isDone()){this.__pu();
}},__pu:function(){var be;
...
this.fireEvent(bf);
},_setPhase:function(bg){var bh=this.__pr;
this.__pr=bg;
...
B+=this.__pv(name,z,w);
}}else if(qx.lang.Type.isDate(y)&&x!=null){B+=this.__pv(name,x.format(y),w);
}else{B+=this.__pv(name,y,w);
}}return B.substring(0,B.length-1);
},__pv:function(name,D,E){if(D instanceof qx.core.Object&&E!=null){var F=encodeURIComponent(E(D));
...
},init:true}},members:{__pw:null,_createTransport:function(){return new qx.bom.request.Xhr();
...
if(typeof B.PARSER[A]===o){return this.__pw=B.PARSER[A];
}return this.__pw=A;
},_getParser:function(){var C=this.__pw,D;
if(C){return C;
...
qx.Bootstrap.define(p,{construct:function(){this.__px=qx.Bootstrap.bind(this.__pJ,this);
this.__py=qx.Bootstrap.bind(this.__pI,this);
this.__pn=qx.Bootstrap.bind(this.__pN,this);
this.__pH();
if(window.attachEvent){this.__pz=qx.Bootstrap.bind(this.__pQ,this);
window.attachEvent(a,this.__pz);
}},statics:{UNSENT:0,OPENED:1,HEADERS_RECEIVED:2,LOADING:3,DONE:4},members:{readyState:0,responseText:i,responseXML:null,status:0,statusText:i,timeout:0,open:function(x,y,z,A,B){this.__pS();
...
if(!this.__pR()&&this.readyState>qx.bom.request.Xhr.UNSENT){this.dispose();
this.__pH();
}this.__pF.onreadystatechange=this.__px;

try{if(qx.core.Environment.get(j)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Open native request with method: "+x+", url: "+y+", async: "+z);
}this.__pF.open(x,y,z,A,B);
}catch(C){if(!qx.util.Request.isCrossDomain(y)){throw C;
}
if(!this.__pD){this.__pE=C;
}
if(this.__pD){if(window.XDomainRequest){this.readyState=4;
this.__pF=new XDomainRequest();
this.__pF.onerror=qx.Bootstrap.bind(function(){this.onreadystatechange();
this.onerror();
this.onloadend();
},this);
...
return;
}window.setTimeout(qx.Bootstrap.bind(function(){if(this.__oi){return;
}this.readyState=4;
this.onreadystatechange();
this.onerror();
this.onloadend();
},this));
}}if(qx.core.Environment.get(k)===d&&parseInt(qx.core.Environment.get(b),10)<2&&!this.__pD){this.readyState=qx.bom.request.Xhr.OPENED;
this.onreadystatechange();
}},setRequestHeader:function(D,E){this.__pS();
if(D==s||D==t||D==v||D==n){this.__pB=true;
}this.__pF.setRequestHeader(D,E);
},send:function(F){this.__pS();
if(!this.__pD&&this.__pE){throw this.__pE;
}if(qx.core.Environment.get(k)===f&&this.timeout===0){this.timeout=10000;
}if(this.timeout>0){this.__pG=window.setTimeout(this.__pn,this.timeout);
}F=typeof F==h?null:F;
try{if(qx.core.Environment.get(j)){qx.Bootstrap.debug(qx.bom.request.Xhr,"Send native request");
}this.__pF.send(F);
}catch(H){if(!this.__pD){throw H;
}if(this._getProtocol()===c){this.readyState=2;
this.__pK();
var G=this;
window.setTimeout(function(){if(G.__oi){return;
}G.readyState=3;
G.__pK();
G.readyState=4;
G.__pK();
});
}}if(qx.core.Environment.get(k)===d&&!this.__pD){this.__pJ();
}this.__pA=true;
},abort:function(){this.__pS();
this.__pq=true;
this.__pF.abort();

if(this.__pF){this.readyState=this.__pF.readyState;
}},onreadystatechange:function(){},onload:function(){},onloadend:function(){},onerror:function(){},onabort:function(){},ontimeout:function(){},getResponseHeader:function(I){this.__pS();
return this.__pF.getResponseHeader(I);
},getAllResponseHeaders:function(){this.__pS();
return this.__pF.getAllResponseHeaders();
},getRequest:function(){return this.__pF;
},dispose:function(){if(this.__oi){return false;
}window.clearTimeout(this.__pG);
if(window.detachEvent){window.detachEvent(a,this.__pz);
}try{this.__pF.onreadystatechange;
}catch(K){return;
}var J=function(){};
this.__pF.onreadystatechange=J;
this.__pF.onload=J;
this.__pF.onerror=J;
this.abort();
this.__pF=null;
...
},_getProtocol:function(){var M=this.__pC;
var N=/^(\w+:)\/\//;
if(M!==null&&M.match){var O=M.match(N);

if(O&&O[1]){return O[1];
}}return window.location.protocol;
},__pF:null,__pD:null,__px:null,__py:null,__pz:null,__pn:null,__pA:null,__pC:null,__pq:null,__eH:null,__oi:null,__pG:null,__pE:null,__pB:null,__pH:function(){this.__pF=this._createNativeXhr();
this.__pF.onreadystatechange=this.__px;
if(this.__pF.onabort){this.__pF.onabort=this.__py;
}this.__oi=this.__pA=this.__pq=false;
},__pI:function(){if(!this.__pq){this.abort();
}},__pJ:function(){var P=this.__pF,Q=true;
...
if(this.readyState===qx.bom.request.Xhr.DONE&&this.__pq&&!this.__pA){return;
}if(!this.__pD&&(P.readyState==2||P.readyState==3)){return;
}this.status=0;
...
}}},__pK:function(){var S=this;
if(this.readyState===qx.bom.request.Xhr.DONE){window.clearTimeout(this.__pG);
}if(qx.core.Environment.get(k)==g&&qx.core.Environment.get(b)<8){if(this.__pD&&!this.__pA&&this.readyState>=qx.bom.request.Xhr.LOADING){if(this.readyState==qx.bom.request.Xhr.LOADING){return ;
}
if(this.readyState==qx.bom.request.Xhr.DONE){window.setTimeout(function(){if(S.__oi){return;
}S.readyState=3;
...
if(this.readyState===qx.bom.request.Xhr.DONE){this.__pL();
}},__pL:function(){if(this.__eH){this.ontimeout();
if(qx.core.Environment.get(k)===f){this.onerror();
}this.__eH=false;
}else{if(this.__pq){this.onabort();
}else{this.__pM()?this.onerror():this.onload();
}}this.onloadend();
},__pM:function(){var T;
if(this._getProtocol()===c){T=!this.responseText;
}else{T=!this.statusText;
}return T;
},__pN:function(){var U=this.__pF;
...
this.responseXML=null;
this.__pK();
},__pO:function(){var V=this.readyState===qx.bom.request.Xhr.DONE;
if(this._getProtocol()===c&&this.status===0&&V){if(!this.__pM()){this.status=200;
}}if(this.status===1223){this.status=204;
}if(qx.core.Environment.get(k)===f){if(V&&this.__pB&&!this.__pq&&this.status===0){this.status=304;
}}},__pP:function(){if(qx.core.Environment.get(k)==g&&(this.getResponseHeader(l)||i).match(/[^\/]+\/[^\+]+\+xml/)&&this.responseXML&&!this.responseXML.documentElement){var W=new window.ActiveXObject(o);
W.async=false;
W.validateOnParse=false;
W.loadXML(this.responseText);
this.responseXML=W;
}},__pQ:function(){try{if(this){this.dispose();
}}catch(e){}},__pR:function(){var name=qx.core.Environment.get(k);
var X=qx.core.Environment.get(q);
return !(name==g&&X<9||name==d&&X<3.5);
},__pS:function(){if(this.__oi){throw new Error("Already disposed");
...
qx.Class.define(f,{extend:tree.ui.popup.Base,construct:function(h){this.__pT=h;
tree.ui.popup.Base.call(this,this.tr(g));
this.setShowClose(true);
},members:{__pT:null,_constructInterface:function(){if(this.__pT===null||this.__pT.trim()==e){this.__pT=this.tr(a);
}this.add(new qx.ui.basic.Label(this.__pT));
...
},__pW:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__pX:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__pX,this);
return;
...
if(this.__pV){this.__pV.call(this);
delete this.__pV;
...
if(this.__pU){bo.cancelOpen(this.__pU);
this.__pU=null;
...
}if(this.__pU){br.cancelOpen(this.__pU);
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="blur",o="Enter",n="__jG",m="__qa",l="Up",d="__pY",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
this.__pY.addListener(t,this._onOpenInterval,this);
this.__qa=new qx.event.Timer;
this.__qa.addListener(t,this._onCloseInterval,this);
},members:{__qb:null,__qc:null,__pY:null,__qa:null,__jG:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
if(L.isVisible()){if(this.__qb){this.cancelOpen(this.__qb);
}}else if(this.__qb!=L){this.__qb=L;
this.__pY.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__qc){this.cancelClose(this.__qc);
}}else if(this.__qc!=M){this.__qc=M;
this.__qa.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__qb==N){this.__pY.stop();
this.__qb=null;
}},cancelClose:function(O){if(this.__qc==O){this.__qa.stop();
this.__qc=null;
}},_onOpenInterval:function(e){this.__pY.stop();
this.__qb.open();
this.__qb=null;
},_onCloseInterval:function(e){this.__qa.stop();
this.__qc.exclude();
this.__qc=null;
...
}},__qd:{"Enter":1,"Space":1},__mN:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__mN[R]||(this.__qd[R]&&Q.getSelectedButton())){e.stopPropagation();
...
}},__qe:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
}else if(this.__qf.length>0){do{var M=this.__qf[0];
...
var K=false;
if(this.__qf.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__qh(M);
B+=F;
if(C&&this.__qf.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__qf.length>0);
}},__qh:function(N){N.setVisibility(w);
this.__qf.shift();
this.fireDataEvent(p,N);
},__qi:function(O){if(!O){return;
}this.__qf.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__qg.length-1;i>=0;i--){var P=this.__qg[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__qg[T]!=undefined){throw new Error("Priority already in use!");
}this.__qg[T]=S;
...
}for(var i=0;i<this.__qf.length;i++){this.__qf[i].setVisibility(w);
}this.__qf=[];
...
}},__qj:false,_setAllowMenuOpenHover:function(bc){this.__qj=bc;
},_isAllowMenuOpenHover:function(){return this.__qj;
...
s.addListener(b,this.__qk,this);
this._add(s);
break;
...
},__qk:function(){var w=this.getChildrenContainer().getChildren();
...
},properties:{valid:{check:n,init:false,event:h},text:{init:u,event:j},fieldNameValue:{init:u,event:l}},events:{'continue':i},members:{__ql:null,_constructInterface:function(){var z=new qx.ui.basic.Label(u);
this.add(z);
this.bind(d,z,r);
...
y.addListener(g,this.__qm,this);
y.focus();
this.bind(k,y,r);
...
this.bind(m,w,p);
},__qm:function(){var A=false;

if(this.__ql.getValue().trim().length>0){A=true;
}this.setValid(A);
}}});
})();
...
if(bk&&this.__qn){this.__qs().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__qn){this.__qs().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__qn){if(bs){this._showPlaceholder();
}else{this._removePlaceholder();
}}else{var bu=this.getContentElement();
bu.setAttribute(a,bs?this.getPlaceholder():j);
}},__qq:{width:16,height:16},_getContentHint:function(){return {width:this.__qq.width*10,height:this.__qq.height||16};
...
if(this.__qn){this.__qs().setStyles(bx);
}if(bv){this.__qq=qx.bom.Label.getTextSize(d,bx);
}else{delete this.__qq;
}qx.ui.core.queue.Layout.add(this);
...
this.selectAllText();
},_getTextSize:function(){return this.__qq;
},_onHtmlInput:function(e){var bD=e.getData();
var bC=true;
this.__qo=false;
if(this.__nM&&this.__nM===bD){bC=false;
...
}if(bC){this.fireDataEvent(be,bD,this.__nM);
this.__nM=bD;
if(this.getLiveUpdate()){this.__qr(bD);
}}},_onWebFontStatusChange:function(bF){if(bF.getData().valid===true){var bG=this.__jK.getStyles();
this.__qq=qx.bom.Label.getTextSize(d,bG);
qx.ui.core.queue.Layout.add(this);
}},__qr:function(bH){var bI=this.__nL;
this.__nL=bH;

if(bI!=bH){this.fireNonBubblingEvent(o,qx.event.type.Data,[bH,bI]);
}},setValue:function(bJ){if(bJ===null){if(this.__qo){return bJ;
}bJ=j;
this.__qo=true;
}else{this.__qo=false;
if(this.__qn){this._removePlaceholder();
}}
...
bL.setValue(bJ);
var bK=this.__qo?null:bJ;
this.__nL=bM;
this.__qr(bK);
}if(this.__qn){this._showPlaceholder();
}return bJ;
}throw new Error("Invalid value type: "+bJ);
},getValue:function(){var bN=this.getContentElement().getValue();
return this.__qo?null:bN;
},resetValue:function(){this.setValue(null);
},_onChangeContent:function(e){this.__qo=e.getData()===null;
this.__qr(e.getData());
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__qs().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__qs().setStyle(bc,L);
}},__qs:function(){if(this.__qp==null){this.__qp=new qx.html.Label();
var bS=qx.theme.manager.Color.getInstance();
this.__qp.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bS.resolve(I)});
this.getContainerElement().add(this.__qp);
}return this.__qp;
},_onChangeLocale:qx.core.Environment.select(g,{"true":function(e){var content=this.getPlaceholder();

if(content&&content.translate){this.setPlaceholder(content.translate());
}},"false":null}),_applyPlaceholder:function(bT,bU){if(this.__qn){this.__qs().setValue(bT);
...
}}},destruct:function(){this.__qp=this.__jK=null;
...
this.__qt=o;
},members:{__qt:null,__qu:null,__qv:null,_createDomElement:function(){return qx.bom.Input.create(this.__qt);
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__qv=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__qu?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__qu=w;
qx.html.Element.prototype.setSelectable.call(this,this.__qv&&w);
...
},setWrap:function(B,C){if(this.__qt===l){this._setProperty(n,B,C);
}else{throw new Error("Text wrapping is only support by textareas!");
}return this;
},getWrap:function(){if(this.__qt===l){return this._getProperty(n);
}else{throw new Error("Text wrapping is only support by textareas!");
}}}});
})();
...
(function(){var n='undefined',m="",k='</span></div></div>',j='.',h='<div class="wrapper" id="preview-',g='body',f='unselectable',d="html/cells",c="data-item-coordinates='",b="error",bn='"></div><div class="label"><span>',bm='updated',bl="default",bk="data-item-coordinates",bj='changeState',bi='_applyItems',bh=',',bg="auto",bf='success',be='loadend',u="'",v='" height="',s='"><canvas width=20 height=20></canvas><div class="img-wrapper"><img src="" height="" width=""></div><div class="label"><span>',t="dragexit",q='changeNode',r='complete',o="move",p='_applyNode',A="drop",B="layer",J="abort",H="resize",R='Edit',M='running',ba='gallery',W="dragstart",D='itemsloaded',bd="multi",bc='appear',bb='"><canvas width=20 height=20></canvas><div class="img-wrapper"><img src="',C="dragenter",F="off",G='" width="',I='execute',K=' ',N='thumbnail ',T='qx.event.type.Event',Y='',w="manager",x="droprequest",E='#preview-',Q="position: absolute;",P='/',O='thumbnail',V="tree.ui.gallery.Gallery",U="dragend",L="mouse",S="dragover",a='thumbnail-selected',X="text-align: center;",y='mouseup',z=",";
qx.Class.define(V,{extend:qx.ui.container.Composite,events:{itemsloaded:T},properties:{items:{init:null,apply:bi},node:{init:null,nullable:true,apply:p,event:q}},construct:function(bo,bp){qx.ui.container.Composite.call(this,bo);
this.__qw={};
this.__qx=bp;
this.addListener(y,this._onMouseUp,this);
this.addListenerOnce(bc,function(){var bs=this.getContainerElement().getDomElement();
bs.addEventListener(A,qx.lang.Function.bind(this._onDrop,this));
bs.addEventListener(C,this._noopHandler,false);
bs.addEventListener(t,this._noopHandler,false);
bs.addEventListener(S,this._noopHandler,false);
},this);
this.setLayout(new qx.ui.layout.Grow());
...
this.itemWidth=110;
this.itemCount=0;
var bq=this._createScroller();
this.__qy=bq;
bq.set({scrollbarX:F,scrollbarY:bg});
this.add(bq);
bq.getPane().addListener(H,this._onPaneResize,this);
this.manager=new qx.ui.virtual.selection.CellRectangle(bq.getPane(),this).set({mode:bd,drag:false});
this.manager.attachMouseEvents();
this.manager.attachKeyEvents(bq);
var br=qx.theme.manager.Font.getInstance().resolve(bl).getStyles();
this._fontCss=qx.bom.element.Style.compile(br);
this.initItems();
this.setDraggable(true);
this.addListener(W,function(e){var bt=this.manager.getSelection();

for(var i=0,l=bt.length;i<l;i++){if(typeof (this.getItemData(bt[i].row,bt[i].column).uploadId)!==n){e.preventDefault();
return;
}}e.addAction(o);
e.addType(d);
qx.bom.Collection.query(g).addClass(f);
});
this.addListener(U,function(e){qx.bom.Collection.query(g).removeClass(f);
});
this.addListener(x,function(e){var bv=e.getCurrentType();
var bw=[];

if(bv===d){var bu=this.manager.getSelection();

for(var i=0,l=bu.length;i<l;i++){bw.push(this.getItemData(bu[i].row,bu[i].column));
}}e.addData(bv,bw);
},this);
},members:{__qy:null,__qx:null,__qw:null,_contextMenu:null,selectItem:function(bx){var by=null;
this.getItems().forEach(function(bz){if(bz.itemPk==bx){by=bz;
return false;
}});
this.manager.selectItem(this.getDataForItem(by));
},getDataForItem:function(bA){var bC=this.getItems().indexOf(bA);
var bD=parseInt(bC/this.itemPerLine);
var bB=bC-(bD*this.itemPerLine);
return {row:bD,column:bB};
},_thumbnailPreview:function(bE,bF,bG,event){var bH=new Image();
bH.src=bE.result;
bH.onload=qx.lang.Function.bind(function(){var bL=bH.height;
var bI=bH.width;
bH=null;

if(bL>82){var bJ=bL/82;
bL=parseInt(bL/bJ);
bI=parseInt(bI/bJ);
}
if(bI>90){var bJ=bI/90;
bL=parseInt(bL/bJ);
bI=parseInt(bI/bJ);
}var bK=h+bG+bb+event.target.result+v+bL+G+bI+bn+this._getTitleForFile(bF)+k;
this._addPreview(bG,bK);
},this);
},_getTitleForFile:function(bM){var bN=bM.name.split(j);
bN.splice(-1,1);
return bN.join(j);
},_noThumbnailPreview:function(bO,bP){var bQ=h+bP+s+this._getTitleForFile(bO)+k;
this._addPreview(bP,bQ);
},_addPreview:function(bR,bS){var bT={html:bS,uploadId:bR};
this.__qw[this.getNode().getPk()][bR].item=bT;
this.getItems().push(bT);
this.updateItems();
},_noopHandler:function(bU){bU.stopPropagation();
bU.preventDefault();
},_onDrop:function(e){e.stopPropagation();
e.preventDefault();
var cc=e.dataTransfer;
var bV=cc.files;
var bX=0;
var bY=1*1024*1024;

for(var i=0;i<bV.length;i++){var ca=bV[i];
var cb=Math.ceil(Math.random()*10000).toString();
var bW=new tree.io.Uploader(ca,{node:this.getNode().getPk()});

if(typeof (this.__qw[this.getNode().getPk()])==n){this.__qw[this.getNode().getPk()]={};
}this.__qw[this.getNode().getPk()][cb]={uploader:bW,item:null,progress:new tree.ui.Progress(bW)};
bX+=ca.size;

if(bX>bY){this._noThumbnailPreview(ca,cb);
}else{var cd=new FileReader();
if(cd.addEventListener){cd.addEventListener(be,qx.lang.Function.bind(this._thumbnailPreview,this,cd,ca,cb),false);
}else{cd.onloadend=qx.lang.Function.bind(this._thumbnailPreview,this,cd,ca,cb);
}cd.readAsDataURL(ca);
}bW.addListener(bj,qx.lang.Function.bind(function(ce,cf,cg,event){var ch=event.getData();

if(ch==r){qx.util.TimerManager.getInstance().start(function(){var cj=cg.getItem();
var ck=this.getItems();
var ci=false;

for(var i=0,l=ck.length;i<l;i++){if(ck[i].uploadId==cf){ck.splice(i,1,cj);
ci=true;
break;
}}this.__qz(ce,cf);

if(ci===true){this.updateItems();
}},0,this,null,1000);
}else if([b,J].indexOf(ch)!==-1){this.__qz(ce,cf);
}},this,this.getNode(),cb,bW),this);
bW.start();
}return false;
},__qz:function(cl,cm){this.__qw[cl.getPk()][cm].uploader.dispose();
delete this.__qw[cl.getPk()][cm];
},updateItems:function(){var cq=this.getItems();
this.itemCount=cq!==null?cq.length:0;
var cp=this.__qy.getPane();
var cn=this.__qy.getPaneSize()!==null?this.__qy.getPaneSize().width:this.getWidth();
var co=Math.floor(cn/this.itemWidth);
this.itemPerLine=co;
var cr=Math.ceil(this.itemCount/co);
cp.getColumnConfig().setItemCount(co);
cp.getRowConfig().setItemCount(cr);
cp.fullUpdate();
},_applyItems:function(cs,ct){if(this.getNode()!==null){var cu=this.__qw[this.getNode().getPk()];

if(typeof (cu)!==n){for(var cv in cu){if(cu[cv].uploader.getState()==M){this.getItems().push(cu[cv].item);
}}}}this.manager.clearSelection();
this.updateItems();
this.fireEvent(D);
},_applyNode:function(cw,cx){if(cx!==null){var cy=this.__qw[cx.getPk()];

if(typeof (cy)!==n){for(var cz in cy){cy[cz].progress.dispose();
cy[cz].progress=null;
}}}
if(cw!==null){this.refreshNodeItems();
var cy=this.__qw[cw.getPk()];

if(typeof (cy)!==n){for(cz in cy){var cA=this.__qw[this.getNode().getPk()][cz];
cA.progress=new tree.ui.Progress(cA.uploader);
}}}},refreshNodeItems:function(){var cB=new tree.io.rest.Item();
cB.addListener(bf,function(event){this.setItems(event.getData());
},this);
cB.getForNode({id:this.getNode().getPk()});
},_createScroller:function(){var cC=new qx.ui.virtual.core.Scroller(1,this.itemPerLine,this.itemHeight,this.itemWidth);
this.layer=new tree.ui.virtual.layer.HtmlCell(this);
cC.getPane().addLayer(this.layer);
this.layer.addListener(bm,this._onLayerUpdated,this);
return cC;
},getItemData:function(cD,cE){return this.getItems()[cD*this.itemPerLine+cE];
},isItemSelectable:function(cF){return !!this.getItemData(cF.row,cF.column);
},_onMouseUp:function(event){if(typeof (on_item_click)===n){return;
}var cI=null;
var cK=qx.bom.Collection.create(event.getOriginalTarget());

if(cK.getClass().split(K).indexOf(O)!==-1){var cJ=cK.getAttribute(bk).split(bh);
cI=this.getItemData(parseInt(cJ[0]),parseInt(cJ[1]));

if(typeof (cI.uploadId)===n){cI.path=this.__qx.getCurrentPath()+P+cI.itemPk;
}}
if(cI===null){return;
}
if(event.isRightPressed()){if(this._contextMenu===null){this._contextMenu=new qx.ui.menu.Menu;
var cH=new qx.ui.menu.Button(this.tr(R),null);
this._contextMenu.add(cH);

if(typeof (on_item_edit)===n){cH.setEnabled(false);
}else{cH.addListener(I,function(){on_item_edit(cI);
},this);
}}this._contextMenu.setPlaceMethod(L);
this._contextMenu.placeToPoint({left:event.getDocumentLeft(),top:event.getDocumentTop()});
this._contextMenu.show();
}else{var cG=event.getNativeEvent();

if(cG.altKey===true||cG.ctrlKey===true||cG.metaKey===true||cG.shiftKey===true){return;
}on_item_click(cI);
}},_onPaneResize:function(e){var cO=e.getTarget();
var cL=e.getData().width;
var cM=Math.floor(cL/this.itemWidth);

if(cM==this.itemsPerLine){return;
}var cN=[];
this.manager.getSelection().forEach(qx.lang.Function.bind(function(cQ){cN.push(this.getItemData(cQ.row,cQ.column));
},this));
this.itemPerLine=cM;
var cP=Math.ceil(this.itemCount/cM);
cO.getColumnConfig().setItemCount(cM);
cO.getRowConfig().setItemCount(cP);
selection=[];
cN.forEach(qx.lang.Function.bind(function(cR){selection.push(this.getDataForItem(cR));
},this));
this.manager.replaceSelection(selection);
},_onLayerUpdated:function(){if(this.getNode()!==null){var cS=this.__qw[this.getNode().getPk()];

if(typeof (cS)!==n){for(var cT in cS){var cU=qx.bom.Collection.query(E+cT);

if(cU.length==1){cS[cT].progress.setElement(cU);
}}}}},styleSelectable:function(cV,cW,cX){this.layer.updateLayerData();
},getCellProperties:function(cY,da){var dd=this.getItemData(cY,da);

if(!dd){return m;
}var dc=this.manager.isItemSelected({row:cY,column:da});
var db=m;
return {style:[Q,X,this._fontCss,db].join(m),classes:N+(dc?a:Y),content:dd.html,attributes:c+cY+z+da+u};
}},destruct:function(){this._disposeObjects(B);
...
this.__qA=r;
this.__qB=r.bind(q,this,q);
this.__pG=qx.util.TimerManager.getInstance().start(this._update,150,this);
r.addListener(h,this._onUploaderChangeState,this);
},members:{__qC:0,__pG:null,__qA:null,__qB:null,_onUploaderChangeState:function(event){switch(event.getData()){case c:break;
case n:break;
case b:break;
}},_update:function(){this.__qC++;
this.__qD();
},_applyElement:function(s,t){this.__qD();
},_applyPercentComplete:function(u,v){this.__qD();
},__qD:function(){if(this.getElement()===null){return;
...
this.__qA.removeBinding(this.__qB);
}});
})();
(function(){var c="interval",b="qx.util.TimerManager",a="singleton";
qx.Class.define(b,{extend:qx.core.Object,type:a,statics:{__qE:[],__qF:{},__pG:0},members:{__qG:false,start:function(d,e,f,g,h){if(!h){h=e||0;
}var j=(new Date()).getTime()+h;
this.self(arguments).__qF[++this.self(arguments).__pG]={callback:d,userData:g||null,expireAt:j,recurTime:e,context:f||this};
this.__qH(j,this.self(arguments).__pG);
return this.self(arguments).__pG;
},stop:function(k){var l=this.self(arguments).__qE;
var length=l.length;

for(var i=0;i<length;i++){if(l[i]==k){l.splice(i,1);
break;
}}delete this.self(arguments).__qF[k];
if(l.length==0&&this.__qG){qx.event.Idle.getInstance().removeListener(c,this.__qI,this);
this.__qG=false;
}},__qH:function(m,n){var p=this.self(arguments).__qE;
var o=this.self(arguments).__qF;
var length=p.length;

for(var i=0;i<length;i++){if(o[p[i]].expireAt>m){p.splice(i,0,n);
break;
}}if(p.length==length){p.push(n);
}if(!this.__qG){qx.event.Idle.getInstance().addListener(c,this.__qI,this);
this.__qG=true;
}},__qI:function(){var t=(new Date()).getTime();
var r=this.self(arguments).__qE;
var s=this.self(arguments).__qF;
...
this.__qH(q.expireAt,v);
}else{delete s[v];
}}if(r.length==0&&this.__qG){qx.event.Idle.getInstance().removeListener(c,this.__qI,this);
this.__qG=false;
}}}});
})();
...
}},members:{__qJ:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qJ;
},getMarkup:function(e){if(this.__qJ){return this.__qJ;
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__qJ:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__qJ;
},_applyStyle:function(){},getMarkup:function(){if(this.__qJ){return this.__qJ;
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__qK=new qx.ui.decoration.css3.BorderImage();

if(p){this.__qL(p);
}}else{this.__qK=new qx.ui.decoration.GridDiv(p);
}
if(q!=null){this.__qK.setInsets(q);
}},properties:{baseImage:{check:d,nullable:true,apply:j},insetLeft:{check:o,nullable:true,apply:n},insetRight:{check:o,nullable:true,apply:n},insetBottom:{check:o,nullable:true,apply:n},insetTop:{check:o,nullable:true,apply:n},insets:{group:[k,l,i,e],mode:f}},members:{__qK:null,getMarkup:function(){return this.__qK.getMarkup();
},resize:function(r,s,t){this.__qK.resize(r,s,t);
},tint:function(u,v){},getInsets:function(){return this.__qK.getInsets();
},_applyInsets:function(w,x,name){var y=g+qx.lang.String.firstUp(name);
this.__qK[y](w);
},_applyBaseImage:function(z,A){if(this.__qK instanceof qx.ui.decoration.GridDiv){this.__qK.setBaseImage(z);
}else{this.__qL(z);
}},__qL:function(B){var G,H,J,I;
this.__qK.setBorderImage(B);
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__qJ:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qJ;
...
this.__qJ=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__qJ;
...
}},__qO:function(K){var O=this._generateBackgroundMarkup(K);
...
K[y]=0;
K[p]=0;
return this.__qM=this._generateBackgroundMarkup(K,O);
},__qP:function(S,T,U){var V=this.getInsets();
T-=V.left+V.right;
U-=V.top+V.bottom;
...
},__qQ:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
